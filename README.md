...
![Spell Sync dashboard sketch: Review and update is the main action](docs/images/dashboard-preview.svg)
...
- You use at least one supported app (Chrome, Firefox, editors, macOS Spelling, JetBrains,
  Obsidian, and more — see [Supported apps](docs/SUPPORTED_APPS.md))
...
install → open a folder → **Start here** → **Review and update**.
...
Then: **Start here** → choose a folder → pick apps → **Create project** → on the
dashboard open **Review and update**. Confirm each preview.

First-run defaults turn on common apps when they are found on your machine. Change them
later under **Targets**. Real-application validation is still growing — see
