...
class TestSafetyCoverageExtras(unittest.TestCase):
    ...
    def test_txn_discard_snapshots(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            discard_txn_snapshots(root, wordlist=wordlist)
    ...
    def test_invalid_type_keeps_partial_config(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(settings_mod, "project_config_path", return_value=path):
    ...
    def test_unknown_key_status(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(data, {})
            self.assertTrue(issues)
            missing = Path(d) / "missing-spell-sync.toml"
    ...
    def test_fingerprint_helpers(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch(
                ...
                self.assertEqual(text_result.status, ReadStatus.EMPTY)
