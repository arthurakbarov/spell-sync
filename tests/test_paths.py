...
class TestPathsHelpers(unittest.TestCase):
    ...
    def test_firefox_dict_paths_oserror_during_iteration(self):
        base = Path("/fake/firefox")

        def boom_iter(_self):
            yield Path("/fake/firefox/profile")
            raise OSError("denied during iteration")

        with (
            patch.object(paths_mod, "firefox_profiles_dir", return_value=base),
            patch.object(Path, "is_dir", return_value=True),
            patch.object(Path, "iterdir", boom_iter),
        ):
            self.assertEqual(paths_mod.firefox_dict_paths(), [])
