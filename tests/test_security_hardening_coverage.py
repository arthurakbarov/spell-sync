    ...
    def test_operation_lock_context_json(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("spell_sync.mutation_guards.emit_json") as emit:
                ...
                self.assertNotIn("/", str(payload["detail"]))
                self.assertNotIn(str(tmp), str(payload["detail"]))
    ...
    def test_operation_lock_context_human(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("spell_sync.mutation_guards.log.abort") as abort:
                ...
                message = abort.call_args[0][0]
                self.assertIn(".spell-sync.lock", message)
...
class TestOperationLockMkdirFailure(unittest.TestCase):
    def test_lock_parent_mkdir_failure(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch.object(Path, "mkdir", side_effect=OSError(errno.EROFS, "ro", str(tmp))):
                with self.assertRaises(OperationLockRejected) as raised:
                    with acquire_operation_lock(wordlist, "push"):
                        pass
            self.assertEqual(raised.exception.code, "lock_dir_unwritable")
            self.assertNotIn(str(tmp), raised.exception.detail)
