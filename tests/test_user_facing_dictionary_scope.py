...
class TestUserFacingDictionaryScope(unittest.TestCase):
    ...
    def test_push_report_summary_uses_custom_dictionaries(self):
        ...
        self.assertNotIn("spell-sync-words.txt", report.summary)
        editor_report = build_push_operation_report(
            PushExecution(
                prepared=preview.prepared,
                push_preview=preview,
                result=PushResult(word_count=1, written=("editor:cursor",)),
                outcome=OperationOutcome.COMPLETED,
                message="ok",
            )
        )
        self.assertIn("spell-sync-words.txt", editor_report.summary)
