...
class TestSyncRun(unittest.TestCase):
    ...
    def test_pull_unions_words(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            before, after = pull_into_wordlist(run)
    ...
    def test_push_overwrites_dictionaries(self):
        with tempfile.TemporaryDirectory() as d:
            run, wordlist, path_a, _path_b = self._run(d)
            words = ["alpha", "beta"]
...
class TestSyncRunEdgeCases(unittest.TestCase):
    ...
    def test_wordlist_needs_rewrite_when_unreadable(self):
        from spell_sync.push_setup import wordlist_needs_rewrite

        with tempfile.TemporaryDirectory() as d:
            wordlist = os.path.join(d, "wordlist.txt")
            Path(wordlist).write_text("alpha\x00\n", encoding="utf-8")
            run = make_sync_run(wordlist, dictionaries=[])
            self.assertTrue(wordlist_needs_rewrite(run.context, set()))

    def test_check_wordlist_missing_file_is_not_empty(self):
        with tempfile.TemporaryDirectory() as d:
            wordlist = os.path.join(d, "wordlist.txt")
            run = make_sync_run(wordlist, dictionaries=[])
            self.assertEqual(run.check_wordlist(), ExitCode.WORDLIST_UNREADABLE)
            self.assertEqual(run.status_diffs(), [])
            self.assertIsNone(run.destructive_push_risk())

    def test_status_diffs_unreadable_wordlist_invents_nothing(self):
        with tempfile.TemporaryDirectory() as d:
            wordlist = os.path.join(d, "wordlist.txt")
            path_a = os.path.join(d, "a.txt")
            Path(wordlist).write_text("alpha\x00\n", encoding="utf-8")
