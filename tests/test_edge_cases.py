...
from tests.runtime_helpers import make_sync_run, pull_add_from
...
class TestIoEdgeCases(unittest.TestCase):
    def test_warn_write_failed_emits_message(self):
        buf = io.StringIO()
        with redirect_stdout(buf):
            io_mod._warn_write_failed("/tmp/x", quiet=False)
        message = buf.getvalue()
        self.assertIn("no write access", message)
        self.assertNotIn("/tmp/", message)

    def test_warn_write_failed_skipped_when_quiet(self):
        buf = io.StringIO()
        with redirect_stdout(buf):
            io_mod._warn_write_failed("/tmp/x", quiet=True)
...
class TestLintWhitelist(unittest.TestCase):
    ...
    def test_get_lint_whitelist_falls_back_when_project_file_unreadable(self):
        lint_mod._whitelist_cache = None
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            wordlist = root / "wordlist.txt"
            wordlist.write_text("ok\n", encoding="utf-8")
            (root / "lint-whitelist.txt").write_bytes(b"\x00")
            words = lint_mod.get_lint_whitelist(wordlist)
            self.assertIsInstance(words, set)
            self.assertTrue(lint_mod.project_lint_whitelist_unreadable(wordlist))
...
class TestSyncRunImportAddFrom(unittest.TestCase):
    def test_pull_add_from_wordlist_unreadable(self):
        ...
        with patch.object(
            ...
            result = pull_add_from(run, "/tmp/source.txt")
    ...
    def test_pull_add_from_missing_source(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            result = pull_add_from(run, os.path.join(d, "missing.txt"))
    ...
    def test_pull_add_from_hunspell_dic(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            run = make_sync_run(wordlist, dictionaries=[])
            result = pull_add_from(run, source)
