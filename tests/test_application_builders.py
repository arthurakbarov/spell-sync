...
from spell_sync.application.dashboard_builders import (
    _max_severity,
...
from spell_sync.application.doctor_builders import build_doctor_snapshot
from spell_sync.application.push_pull_preview_builders import build_push_preview
...
class TestDashboardBuilders(unittest.TestCase):
    ...
    def test_build_status_detail_with_dictionary_row(self):
        ...
        with tempfile.TemporaryDirectory() as d:
            ...
            run.context.config_path = Path(d) / "spell-sync.toml"
    ...
    def test_build_status_detail_wordlist_error_skips_load(self):
        ...
        run.context.config_path = Path("/tmp/missing-spell-sync.toml")
        run.check_wordlist.return_value = ExitCode.PUSH_ABORT
        run.load_wordlist.side_effect = AssertionError("must not load wordlist")
        run.status_diffs.side_effect = AssertionError("must not compute diffs")
        run.skipped_unreadable_dictionary_names.return_value = ()
        run.skipped_corrupt_dictionary_names.return_value = ()
        run.destructive_push_risk.side_effect = AssertionError("must not assess risk")
        run.dictionaries = []
    ...
    def test_build_push_preview_plan_identifier_oserror(self):
        ...
        with patch(
            "spell_sync.application.push_pull_preview_builders.file_content_hash",
    ...
    def test_build_dashboard_issues_cleanup_pending_is_warning(self):
        snapshot = StatusSnapshot(
            wordlist_count=1,
            diffs=(),
            skipped_unreadable=(),
        ...
        validated = self._validated(
            journal_result=JournalLoadResult(JournalLoadStatus.VALID_COMPLETED, MagicMock()),
        )
        with (
            patch(
                "spell_sync.application.dashboard_builders.inspect_workspace_git",
                return_value=None,
            ),
            patch(
                "spell_sync.application.dashboard_builders.optional_app_warn_hints",
                return_value=[],
            ),
        ):
            issues = build_dashboard_issues(validated, snapshot, lock_info=None)
            state = build_dashboard_state(validated, snapshot, lock_info=None)
        codes = {issue.code for issue in issues}
        self.assertIn("cleanup_pending", codes)
        self.assertTrue(
            all(
                issue.severity is DashboardSeverity.WARNING
                for issue in issues
                if issue.code == "cleanup_pending"
            )
        )
        self.assertEqual(state.overall_severity, DashboardSeverity.WARNING)
        self.assertFalse(state.pending_recovery)

    def test_build_dashboard_issues_includes_git_dirty_and_sublime_override(self):
        ...
        with (
            ...
            patch(
                "spell_sync.application.dashboard_builders.workspace_git_dirty_message",
                return_value="Personal workspace Git has uncommitted changes (wordlist.txt).",
            ),
            ...
            patch(
                ...
                return_value="Sublime Text User Preferences define added_words (1), which overrides",
            ),
            patch("spell_sync.dictionary_hints.enable_editors", return_value=False),
        ):
            issues = build_dashboard_issues(validated, snapshot, lock_info=None)
        codes = {issue.code for issue in issues}
        self.assertIn("workspace_git_dirty", codes)
        self.assertIn("sublime_user_override", codes)
        self.assertNotIn("sublime_not_found", codes)
        ...
        with (
            ...
            missing = build_dashboard_issues(validated, snapshot, lock_info=None)
        self.assertIn("sublime_not_found", {issue.code for issue in missing})
        self.assertNotIn("sublime_user_override", {issue.code for issue in missing})
    ...
    def test_format_dashboard_last_operation_branches(self):
        ...
        self.assertIn("2 dictionaries updated", format_dashboard_last_operation(push))
        ...
        self.assertIn("1 app changed", format_dashboard_last_operation(targets))
    ...
    def test_build_dashboard_application_counts(self):
        ...
        with patch(
            "spell_sync.application.dashboard_builders.discover_setup_targets",
...
class TestStatusAndPreviewBuilders(unittest.TestCase):
    def test_build_status_detail_snapshot(self):
        ...
        run.wordlist_str = "/tmp/w.txt"
        run.context.project_dir = Path("/tmp")
        run.context.config_path = Path("/tmp/spell-sync.toml")
        run.check_wordlist.return_value = None
...
class TestDoctorBuilder(unittest.TestCase):
    ...
    def test_build_doctor_snapshot_extra_groups_and_actions(self):
        ...
        report = DoctorReport(
            ...
            checks=(
                ...
                DoctorCheck("error", "corrupt push journal found. Run recover."),
        ...
        snapshot = build_doctor_snapshot(report)
        journal_check = next(c for c in snapshot.checks if "journal" in c.detail.lower())
        self.assertEqual(journal_check.suggested_action, "spell-sync recover")
        snapshot = build_doctor_snapshot(report)
        groups = {check.group for check in snapshot.checks}
        self.assertIn("Interrupted update", groups)
    ...
    def test_build_doctor_snapshot_hint_action_and_passed_level(self):
        ...
        snapshot = build_doctor_snapshot(report)
        self.assertEqual(snapshot.checks[0].level, "info")
