...
class TestDoctorPayloadInvariants(unittest.TestCase):
    ...
    def test_git_hooks_payload_shape(self):
        ...
        self.assertEqual(
            set(payload),
            {"pre_push", "pre_commit", "pre_push_stale", "pre_commit_stale"},
        )
...
class TestSkipReasonInvariants(unittest.TestCase):
    def test_push_skip_details_keys_are_known_reasons(self):
        ...
        for reason in (
            ...
            PushSkipReason.CORRUPT,
            PushSkipReason.BACKUP_FAILED,
            PushSkipReason.BLOCKED_BY_USER,
            PushSkipReason.RUNNING_APP,
        ):
            self.assertIn(reason, PUSH_SKIP_DETAILS)
