...
def _prepared_push_mock() -> MagicMock:
    prepared = MagicMock()
    prepared.targets = ()
...
@contextmanager
def _push_preview_run(run: object):
    resolved = MagicMock()
    resolved.context.settings = RuntimeSettings.defaults()
    with patch.object(RuntimeResolver, "resolve_read", return_value=resolved):
        with patch("spell_sync.application.services.sync.sync_run_for", return_value=run):
            yield
...
class TestSpellSyncService(unittest.TestCase):
    ...
    def test_cmd_push_passes_same_preview_to_dry_run_without_reprepare(self):
        ...
        prepared.targets = ()
        ctx = MagicMock()
        ctx.settings = RuntimeSettings.defaults()
        prepared.ctx = ctx
        preview = PushPreview(
            prepared=prepared,
            targets=(),
    ...
    def test_load_dashboard_includes_last_operation_summary(self):
        ...
        self.assertIn("Last: Update my apps", state.last_operation_summary or "")
...
class TestServiceFacadePaths(unittest.TestCase):
    ...
    def test_load_doctor_targets_and_push_plan(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch.object(RuntimeResolver, "sync_run", return_value=run):
                ...
                _preview, diffs, result = service.load_push_plan(
    ...
    def test_load_push_plan_blocked_preview(self):
        ...
        with (
            ...
            _preview, diffs, result = service.load_push_plan(_push("/tmp/w.txt"))
        self.assertEqual(result, ExitCode.PUSH_ABORT)
    ...
    def test_pull_and_push_execution_from_result_paths(self):
        ...
        failed = service.push_execution_from_result(None, ExitCode.PUSH_ABORT)
