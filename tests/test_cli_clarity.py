"""CLI clarity: pull/push verbs, plan, targets, config check."""
...
class TestPlan(unittest.TestCase):
    def test_plan_json_preview(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch_plan_service(load_push_plan=plan):
                buf = io.StringIO()
                with redirect_stdout(buf):
...
class TestTomllibParser(unittest.TestCase):
    ...
    def test_config_check_quiet_log_paths(self):
        with tempfile.TemporaryDirectory() as d:
            project = Path(d) / "spell-sync.toml"
    ...
    def test_doctor_targets_quiet_log_paths(self):
    ...
    def test_doctor_main_quiet_log_success(self):
        with tempfile.TemporaryDirectory() as d:
            wordlist = Path(d) / "wordlist.txt"
            wordlist.write_text("alpha\n", encoding="utf-8")
            run = make_sync_run(str(wordlist), dictionaries=[])
            ...
            with patch_doctor_service(load_doctor_report=report):
                cli_log.quiet = True
                try:
                    code = doctor_mod.cmd_doctor(CliOptions(wordlist=str(wordlist)))
                finally:
                    cli_log.quiet = False
            self.assertEqual(code, int(ExitCode.OK))

    def test_plan_removals_quiet_log_and_wordlist_error(self):
        preview = executable_push_preview()
        preview = PushPreview(
            prepared=None,
            targets=(),
        ...
        empty_preview = executable_push_preview()
        with patch_plan_service(load_push_preview=empty_preview):
            cli_log.quiet = True
            try:
                code = plan_mod.cmd_plan(CliOptions(plan_removals=True))
            finally:
    ...
    def test_plan_preview_wordlist_error_session_fail(self):
        bad = PushPreview(
            prepared=None,
            targets=(),
            additions=0,
        ...
        blocked_plan = (bad, (), ExitCode.WORDLIST_UNREADABLE)
        with patch_plan_service(load_push_plan=blocked_plan):
            buf = io.StringIO()
            with redirect_stdout(buf):
                code = plan_mod.cmd_plan(CliOptions())
        self.assertEqual(code, int(ExitCode.WORDLIST_UNREADABLE))
        self.assertIn("could not read", buf.getvalue().lower())

        prepare_failed = PushPreview(
            prepared=None,
            targets=(),
            additions=0,
            removals=0,
        ...
        blocked_prepare = (prepare_failed, (), ExitCode.PUSH_ABORT)
        with patch_plan_service(load_push_plan=blocked_prepare):
            buf = io.StringIO()
            with redirect_stdout(buf):
                code = plan_mod.cmd_plan(CliOptions())
        self.assertEqual(code, int(ExitCode.PUSH_ABORT))
        self.assertIn("could not build an update preview", buf.getvalue().lower())
        self.assertNotIn("could not read the word list", buf.getvalue().lower())
...
class TestStage3HumanOutput(unittest.TestCase):
    ...
    def test_config_check_human_no_config_files(self):
        missing = Path("/nonexistent/spell-sync-absent.toml")
        with patch.object(settings_mod, "project_config_path", return_value=missing):
            code = config_check_mod.cmd_config_check(CliOptions())
        self.assertEqual(code, int(ExitCode.OK))
