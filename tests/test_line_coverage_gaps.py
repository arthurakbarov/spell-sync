...
from tests.test_sync_facade_coverage import _pull_scope
...
class TestLineCoverageGaps(unittest.TestCase):
    ...
    def test_execute_prepared_pull_fingerprint_unreadable_fails_closed(self):
        with tempfile.TemporaryDirectory() as tmp:
            wordlist = Path(tmp) / "wordlist.txt"
            wordlist.write_text("alpha\n", encoding="utf-8")
            run = make_sync_run(wordlist)
            with patch(
                "spell_sync.push_journal.file_content_hash",
                return_value=None,
            ):
                result = run.execute_prepared_pull(
                    merged_words=("alpha", "beta"),
                    before_count=1,
                    after_count=2,
...
class TestLineCoverageGapsUi(unittest.IsolatedAsyncioTestCase):
    async def test_dashboard_corrupt_journal_banner(self):
        ...
        async with app.run_test(size=(100, 36)) as pilot:
            content = await wait_for_text(pilot, "#blocking-banner", "Corrupt update journal")
    ...
    async def test_recovery_screen_worker_poll_paths(self):
        ...
        async with app.run_test(size=(100, 36)) as pilot:
            ...
            screen._worker = error_worker
            screen.on_load_recovery_worker_state_changed(
                MagicMock(worker=error_worker, state=WorkerState.ERROR)
            )
            ...
            screen._worker = success_worker
            screen.on_load_recovery_worker_state_changed(
                MagicMock(worker=success_worker, state=WorkerState.SUCCESS)
            )
