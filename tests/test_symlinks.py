...
class TestPullReadableGuard(unittest.TestCase):
    def test_pull_skips_unreadable_dictionary_with_warn(self):
        with tempfile.TemporaryDirectory() as tmp:
            ...
            with patch("spell_sync.application.builders.dictionary_read_result") as read:
                from spell_sync.read_outcome import DictionaryReadResult, ReadStatus

                read.return_value = DictionaryReadResult(
                    ReadStatus.UNREADABLE, frozenset(), "blocked", None
                )
            ...
            self.assertTrue(preview.sources_skipped)
            self.assertTrue(
                any("pull skipped" in (row.detail or "") for row in preview.source_rows)
            )
