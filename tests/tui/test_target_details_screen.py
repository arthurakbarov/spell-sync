...
class TestTargetDetails(unittest.IsolatedAsyncioTestCase):
    ...
    def test_automated_validation_from_packaged_data(self) -> None:
        ...
        # Manual pass is recorded for chrome/macos only; linux/windows stay not-run.
        expected_manual = "pass" if sys.platform == "darwin" else "not-run"
        self.assertEqual(details.manual_validation, expected_manual)
