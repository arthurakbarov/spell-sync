"""Coverage for recovery screens and service paths."""
...
class TestRecoveryScreenCoverage(unittest.IsolatedAsyncioTestCase):
    async def test_recovery_refresh_and_discard_flow(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertEqual(str(screen.query_one("#btn-run").label), RECOVERY_DISCARD_LABEL)
    ...
    async def test_recovery_cleanup_pending_labels(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertEqual(str(recover.label), RECOVERY_CLEANUP_LABEL)
            await pilot.click("#btn-recover")
            await wait_for_text(pilot, "#confirm-summary", "Type RECOVER")
            self.assertEqual(str(app.screen.query_one("#btn-run").label), RECOVERY_CLEANUP_LABEL)
    ...
    async def test_recovery_worker_error_callback(self):
        controller = TuiController(fake_service(), CliOptions())
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            app.push_screen(RecoveryScreen(controller))
            ...
            event = MagicMock(worker=worker, state=WorkerState.ERROR)
            screen.on_load_recovery_worker_state_changed(event)
    ...
    async def test_cleanup_confirm_accepts_recover_token(self):
        preview = sample_recovery_preview(
            status=RecoveryStatus.COMPLETED_CLEANUP_PENDING,
            can_cleanup=True,
            can_discard=False,
        )
        controller = TuiController(fake_service(recovery_preview=preview), ProjectRef())
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            screen = app.screen
            assert isinstance(screen, RecoveryConfirmScreen)
            self.assertEqual(screen._typed_token, "RECOVER")
            ...
            self.assertFalse(screen.query_one("#btn-run", Button).disabled)

    async def test_recovery_worker_error_does_not_publish_absent(self):
        controller = TuiController(fake_service(), ProjectRef())
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            screen = RecoveryScreen(controller)
            app.push_screen(screen)
            await pilot.pause()
            with patch.object(controller, "inspect_recovery", side_effect=RuntimeError("boom")):
                fallback = RecoveryScreen.load_recovery_worker.__wrapped__(screen)
            self.assertIsNotNone(fallback.load_error)
            screen._render_preview(fallback)
            self.assertIsNone(controller.active_recovery_preview())
            self.assertTrue(screen.query_one("#btn-recover", Button).disabled)
