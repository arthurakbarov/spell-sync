...
class TestDashboardScreen(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pending_recovery_with_issue(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertIn("unfinished update journal", str(banner.render()).lower())
    ...
    async def test_pending_recovery(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertIn("unfinished update journal", str(banner.render()))
    ...
    async def test_section_to_first_button_gaps_match(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            gaps = [
                ...
                for label, btn in zip(labels, first_buttons)
            ]
    ...
    async def test_menu_item_focus_within_marks_parent(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            css = Path(__file__).resolve().parents[2] / "spell_sync" / "tui" / "app.tcss"
            self.assertIn(".menu-item:focus-within", css.read_text(encoding="utf-8"))
    ...
    async def test_corrupt_journal_banner(self):
        ...
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            banner = await wait_for_text(pilot, "#blocking-banner", "Corrupt update journal")
            self.assertIn("Corrupt update journal", str(banner.render()))
