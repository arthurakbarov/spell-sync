...
class TestScreenCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_dashboard_mount_failure_and_worker_paths(self):
        ...
        async with app.run_test(size=(60, 20)) as pilot:
            await wait_for_text(pilot, "#narrow-warning", "80 by 24")
            screen = app.screen
    ...
    async def test_status_render_branches_and_workers(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            await wait_for_text(pilot, "#status-summary", "large removal set")
            screen = app.screen
            ...
            self.assertIn("No applications configured", _static_text(screen, "#status-summary"))
    ...
    async def test_preview_render_branches_and_workers(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertIsNotNone(fallback.prepare_error)
            self.assertFalse(fallback.is_executable)
...
class TestReportScreenBackNavigation(unittest.TestCase):
    ...
    def test_setup_report_dashboard_branch(self):
        ...
        with patch.object(DashboardScreen, "action_refresh_dashboard") as refresh:
            with patch.object(
                DashboardScreen, "is_mounted", new_callable=PropertyMock, return_value=True
            ):
