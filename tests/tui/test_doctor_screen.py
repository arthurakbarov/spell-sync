...
from spell_sync.application.product_concepts import HEALTH_LABEL
...
class TestDoctorScreen(unittest.IsolatedAsyncioTestCase):
        ...
        doctor = sample_doctor(
            checks=(
                ...
                DoctorCheckView(
                    group="Word list",
                    level="warning",
                    title="Word list empty",
                    detail="No words yet.",
                    suggested_action="Add words",
                ),
                DoctorCheckView(
                    group="Interrupted update",
                    level="failed",
    ...
    async def test_info_level_label_not_passed(self):
        doctor = sample_doctor(
            checks=(
                DoctorCheckView(
                    group="Project",
                    level="info",
                    title="spell-sync not on PATH",
                    detail="Use uv run spell-sync.",
                ),
            ),
            has_errors=False,
        )
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            app.push_screen(DoctorScreen(controller))
            await wait_for_text(pilot, "#doctor-summary", "no blocking")
            table = app.screen.query_one("#doctor-table", DataTable)
            joined = " | ".join(
                " ".join(str(cell) for cell in table.get_row_at(i)) for i in range(table.row_count)
            )
    ...
    async def test_empty_checks_shows_placeholder(self):
        doctor = sample_doctor(checks=(), has_errors=False)
        controller = TuiController(fake_service(doctor=doctor), CliOptions())
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            app.push_screen(DoctorScreen(controller))
            ...
            joined = " ".join(str(cell) for cell in table.get_row_at(0))
            self.assertIn("No issues", joined)
    ...
    async def test_rerun_and_back(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            app.push_screen(DoctorScreen(controller))
            await wait_for_text(pilot, "#doctor-summary", HEALTH_LABEL)
    ...
    async def test_export_support_report_ignores_repeated_click(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            btn = screen.query_one("#btn-export-support", Button)
            for _ in range(100):
                if btn.disabled or started.is_set():
                    break
                await pilot.pause()
            self.assertTrue(btn.disabled or started.is_set())
            await pilot.click("#btn-export-support")
            await wait_for_text(pilot, "#doctor-export-status", "Report saved")
