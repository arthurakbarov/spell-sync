...
from spell_sync.application.product_concepts import (
    RECOVERY_CONFIRM_RECOVER_BUTTON,
    RECOVERY_NONE_FOUND,
)
...
class TestRecoveryFlow(unittest.IsolatedAsyncioTestCase):
    async def test_recovery_screen_absent(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            await wait_for_text(pilot, "#recovery-content", "No interrupted update")
    ...
    async def test_typed_recover_confirmation(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertEqual(
                str(screen.query_one("#btn-run").label), RECOVERY_CONFIRM_RECOVER_BUTTON
            )
    ...
    async def test_recovery_screen_recoverable_notice(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertIn("interrupted update", str(content.render()).lower())
    ...
    async def test_recovery_screen_corrupt_journal_notice(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            content = await wait_for_text(
                pilot, "#recovery-content", "Damaged interrupted-update record"
            )
