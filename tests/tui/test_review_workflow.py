...
class TestReviewWorkflow(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_pull_additions_flow(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            preview_pull = app.screen.query_one("#btn-pull")
            self.assertEqual(str(preview_pull.label), f"{COLLECT_WORDS_LABEL} (+17)")
            await pilot.click("#btn-pull")
            await wait_for_text(pilot, "#confirm-summary", "Add 17 words")
            self.assertEqual(str(app.screen.query_one("#btn-run").label), COLLECT_CONFIRM_BUTTON)
            ...
            self.assertEqual(
                str(app.screen.query_one("#btn-build-push").label),
                CONTINUE_TO_UPDATE_APPS_LABEL,
            )
    ...
    async def test_pull_no_changes_message(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertFalse(screen.query_one("#btn-extra-words").display)
    ...
    async def test_skip_collect_with_additions_and_empty_list_routes_to_add_words(self):
        preview = sample_pull_preview(before_count=0, after_count=17)
        controller = TuiController(fake_service(pull_preview=preview), CliOptions())
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            await self._open_review_pull(pilot, controller)
            skip = app.screen.query_one("#btn-skip")
            self.assertEqual(str(skip.label), "Skip collect")
            await pilot.click("#btn-skip")
            from spell_sync.tui.screens.first_win_screen import AddWordsScreen

            self.assertIsInstance(app.screen, AddWordsScreen)

    async def test_pull_empty_list_routes_to_add_words(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertIn("Add words to your list", str(content.render()))
            skip = app.screen.query_one("#btn-skip")
            self.assertEqual(str(skip.label), "Add words to my list")
    ...
    async def test_empty_list_add_words_continue_stays_in_review(self):
        preview = sample_pull_preview(
            additions=0,
            before_count=0,
            after_count=0,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            write_text_words(str(wordlist), [], "utf-8", False, quiet=True)
            controller = TuiController(fake_service(pull_preview=preview), CliOptions())
            ...
            async with app.run_test(size=(100, 48)) as pilot:
                ...
                self.assertIsInstance(app.screen, AddWordsScreen)
                app.screen.query_one("#add-words-input").text = "AcmeCorp\n"
                await pilot.click("#btn-save")
                await wait_for_text(pilot, "#add-words-status", "Saved to your word list")
                await pilot.click("#btn-update")
                await wait_for_text(pilot, "#review-push-content", "Update my apps")
                self.assertIsInstance(app.screen, ReviewPushScreen)
    ...
    async def test_empty_list_add_words_back_refreshes_collect(self):
        preview = sample_pull_preview(
            additions=0,
            before_count=0,
            after_count=0,
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            async with app.run_test(size=(100, 48)) as pilot:
                ...
                await pilot.click("#btn-save")
                await wait_for_text(pilot, "#add-words-status", "Saved to your word list")
                service.pull_preview = sample_pull_preview(
                    additions=0,
                    before_count=1,
                    after_count=1,
                    addition_words=frozenset(),
                ...
                await wait_for_text(pilot, "#review-pull-content", "Next: Update my apps")
                skip = app.screen.query_one("#btn-skip")
                self.assertEqual(str(skip.label), CONTINUE_TO_UPDATE_APPS_LABEL)
                self.assertIsInstance(app.screen, ReviewPullScreen)

    async def test_review_pull_extra_words_continue_stays_in_review(self):
        with tempfile.TemporaryDirectory() as tmp:
            wordlist = Path(tmp) / "wordlist.txt"
    ...
    async def test_fresh_push_preview_after_pull_execution(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            await pilot.click("#btn-run")
            await dismiss_operation_linger(pilot)
            await wait_for_text(pilot, "#review-pull-complete", "Next: Update my apps")
            await pilot.click("#btn-build-push")
            await wait_for_text(pilot, "#review-push-content", "Update my apps")
    ...
    async def test_push_with_typed_confirmation(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertEqual(
                str(app.screen.query_one("#btn-push").label), CONTINUE_TO_UPDATE_APPS_LABEL
            )
