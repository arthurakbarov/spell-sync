...
from spell_sync.application.operation_reports import (
...
from spell_sync.guest_messages import RECOVERY_DISCARDED_MESSAGE
...
class FakeTuiService:
    ...
    extra_inventory: ExtraWordInventory | None = None
    extra_wipe_result: ExtraWordsWipeResult | None = None
    load_extra_word_inventory_calls: int = 0
    subtract_extra_words_calls: int = 0
    last_subtracted_words: tuple[str, ...] = ()
    ...
    def build_push_report(self, execution: PushExecution, *, duration_ms: int = 0):
        del duration_ms
        return build_push_operation_report(execution)

    def build_pull_report(self, execution: PullExecution, *, duration_ms: int = 0):
        del duration_ms
    ...
    def execute_recovery_discard(
        ...
        *,
        ...
        return RecoveryExecution(
            ...
            message=RECOVERY_DISCARDED_MESSAGE,
        )

    def build_recovery_report(self, execution: RecoveryExecution, *, duration_ms: int = 0):
        del duration_ms
    ...
    def build_setup_report(self, execution: ProjectSetupExecution, *, duration_ms: int = 0):
        del duration_ms
    ...
    def load_extra_word_inventory(self, request: StatusRequest) -> ExtraWordInventory:
        self.load_extra_word_inventory_calls += 1
        if self.extra_inventory is not None:
            return self.extra_inventory
        return ExtraWordInventory.empty(wordlist_path="/tmp/wordlist.txt")
    ...
    def subtract_extra_words(
        ...
        self.subtract_extra_words_calls += 1
        self.last_subtracted_words = words
        if self.extra_wipe_result is not None:
            return self.extra_wipe_result
...
def sample_extra_inventory(*, count: int = 3, **kwargs) -> ExtraWordInventory:
    rows = tuple(
        ExtraWordRow(
            word=f"extra{index:02d}",
            sources=("chrome", "firefox") if index % 2 == 0 else ("chrome",),
        )
    ...
    defaults = dict(
        ...
        rows=rows,
        sources=(
            ExtraWordSource("chrome", "/tmp/chrome.txt", "aaa"),
            ExtraWordSource("firefox", "/tmp/firefox.txt", "bbb"),
        ),
        warnings=(),
        created_at="2026-01-01T00:00:00+00:00",
        wordlist_fingerprint="deadbeef",
    ...
    defaults.update(kwargs)
    return ExtraWordInventory(**defaults)
...
def fake_service(
    *,
    ...
    snapshot = sample_status(empty=empty_wordlist, wordlist_error=wordlist_error)
    labels = {
        DashboardSeverity.READY: "✓ Ready",
        DashboardSeverity.WARNING: "! Needs attention",
    ...
    return FakeTuiService(
        ...
        extra_wipe_result=extra_wipe_result,
    )
