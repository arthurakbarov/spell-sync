...
def _record(**overrides) -> OperationHistoryRecord:
    base = OperationHistoryRecord(
        schema_version=1,
        record_id="rec-1",
        timestamp=datetime(2026, 7, 18, 14, 32, tzinfo=UTC),
        operation="push",
        outcome="completed",
        duration_ms=812,
        updated_targets=5,
...
class TestLogsScreen(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_empty_history_state(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            self.assertFalse(app.screen.query_one("#history-table", DataTable).display)
            self.assertFalse(app.screen.query_one("#btn-clear").display)
    ...
    async def test_technical_json_line_renders_as_table_row(self):
        line = (
            ...
            '"severity":"success","stage":"completed"}'
        )
        controller = self._controller()
    ...
    async def test_unparsed_log_lines_have_no_blank_line_after_label(self):
        controller = self._controller()
        controller._service.read_technical_log_tail = lambda **kwargs: TechnicalLogSnapshot(
            path=Path("/tmp/spell-sync.log"),
            lines=("info operation.started", "success operation.completed"),
        )
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            app.push_screen(TechnicalLogScreen(controller))
            ...
            self.assertIn("Unparsed log lines:\ninfo operation.started", content)
            self.assertNotIn("Unparsed log lines:\n\n", content)
            table = app.screen.query_one("#tech-log-table", DataTable)
    ...
    def test_history_detail_variants(self):
        self.assertEqual(
            _history_detail(_record(operation="pull", outcome="completed", added_words=17)),
            "17 added",
        )
        self.assertEqual(
            _history_detail(_record(operation="setup", outcome="completed", created_files=2)),
            "2 files",
        )
        self.assertEqual(
            _history_detail(_record(operation="recover", outcome="completed", restored_files=3)),
            "3 restored",
        )
    ...
    async def test_history_details_all_fields(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            await pilot.pause()
            content = str(app.screen.query_one("#details-content").render())
            self.assertIn("Dictionaries skipped", content)
    ...
    def test_history_detail_default(self):
        self.assertEqual(
            _history_detail(_record(operation="doctor", outcome="failed", updated_targets=0)),
            "Failed",
        )
