"""Coverage for presentation helper modules."""
...
class TestPathPickerWidget(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_match_list_height_leaves_room_for_options_and_border(self) -> None:
        """height == option_count alone yields an empty bordered box (content rows = 0)."""
        with tempfile.TemporaryDirectory() as tmp:
            root = Path(tmp)
            (root / "alpha").mkdir()
            (root / "beta").mkdir()
            ...
            async with app.run_test(size=(100, 40)) as pilot:
                ...
                self.assertGreaterEqual(
                    option_list.region.height,
                    option_list.option_count + WordlistPathPicker._LIST_BORDER_ROWS,
                )
                prompts = [
                    str(option_list.get_option_at_index(i).prompt)
                    for i in range(option_list.option_count)
                ]
                self.assertTrue(any("alpha" in p for p in prompts))
                self.assertTrue(any("beta" in p for p in prompts))
...
class TestConfirmAndRemovals(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_removals_back_button(self) -> None:
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            await pilot.pause()
            self.assertIn("1. alpha", str(app.screen.query_one("#removals-content").render()))
            self.assertIn("1 word", str(app.screen.query_one("#removals-summary").render()))
