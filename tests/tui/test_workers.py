...
class TestWorkers(unittest.IsolatedAsyncioTestCase):
    ...
    def test_worker_state_changed_routes_to_named_handler(self):
        from textual.worker import WorkerState

        class _Routed(LoadTokenMixin):
        ...
        for state in (WorkerState.PENDING, WorkerState.RUNNING, WorkerState.SUCCESS):
            fake = type(
                ...
                (),
                {
                    "worker": type("W", (), {"name": "load_dashboard_worker"})(),
                    "state": state,
                },
            )()
            screen.on_worker_state_changed(fake)  # type: ignore[arg-type]
        self.assertEqual(screen.seen, [WorkerState.SUCCESS])
    ...
    async def test_loading_state(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            done = await wait_for_text(pilot, "#dashboard-summary", "Ready")
            self.assertNotIn("Loading", str(done.render()))

    async def test_back_to_dashboard_from_session_report_refreshes(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            await wait_for_text(pilot, "#dashboard-summary", "Ready")
            app.push_screen(ReviewSessionReportScreen(controller))
            await pilot.pause()
