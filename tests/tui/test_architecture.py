...
class TestTuiArchitecture(unittest.TestCase):
    ...
    def test_path_picker_continue_uses_commit_helper(self):
        """Continue must not raw-read path_value; list highlight needs a commit seam."""
        import re

        source = (Path(tui_pkg.__file__).parent / "screens" / "setup_welcome_screen.py").read_text(
            encoding="utf-8"
        )
        self.assertIn("def commit_path_picker_value", source)
        self.assertGreaterEqual(source.count("commit_path_picker_value("), 3)
        banned = re.compile(r"(?:raw|path)\s*=\s*self\._path_picker\(\)\.path_value\b")
        self.assertIsNone(
            banned.search(source),
            msg="Continue handlers must use commit_path_picker_value, not raw path_value",
        )
        # Custom mode must not leave the preset-only primary as the Enter target.
        self.assertIn("recommended.display = False", source)
        self.assertIn('continue_btn.variant = "primary"', source)
