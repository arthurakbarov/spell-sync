...
class TestTargetSettingsScreenCoverage(unittest.IsolatedAsyncioTestCase):
    ...
    async def test_focus_navigation_and_escape(self):
    ...
    async def test_controller_target_settings_helpers(self):
        ...
        prior = controller.target_settings_discovery()
        self.assertIsNotNone(prior)
        prior_ids = frozenset(t.identifier for t in prior.targets)
        ...
        kept = controller.target_settings_discovery()
        self.assertIsNotNone(kept)
        self.assertEqual(frozenset(t.identifier for t in kept.targets), prior_ids)
    ...
    async def test_button_handlers(self):
        ...
        async with app.run_test(size=(100, 48)) as pilot:
            ...
            screen = app.screen
            assert isinstance(screen, TargetSettingsScreen)
            self.assertEqual(len(screen.query("#btn-refresh")), 0)
    ...
    async def test_selection_screen_escape(self):
        controller = self._controller()
        app = SpellSyncApp(controller)
        async with app.run_test(size=(100, 48)) as pilot:
            await app.push_screen(TargetSettingsScreen(controller))
    ...
    async def test_row_widget_meta_without_path(self):
        ...
        self.assertIn("Not detected", row._meta_line())
