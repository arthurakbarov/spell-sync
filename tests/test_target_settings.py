...
def mock_targets():
    ...
    def _factory(enabled: frozenset[str]):
        def _discover(*, enabled_targets=None):
...
def test_resolve_enabled_skips_non_config_target_ids() -> None:
    unknown = _target("not-a-config-target", selectable=True)
    enabled = resolve_enabled_targets(
        (unknown,),
        selected_target_ids=frozenset({"not-a-config-target"}),
...
def test_resolve_enabled_preserves_corrupt_enabled(
    ...
    _wordlist, _config = _write_config(tmp_path, enabled=("chrome",))
    with mock_targets(frozenset({"chrome"})):
...
def test_resolve_enabled_preserves_absent_previous_family() -> None:
    """A config-enabled family missing from discovery must not be auto-disabled."""
    chrome = _target("chrome", selectable=True)
    enabled = resolve_enabled_targets(
        (chrome,),
        selected_target_ids=frozenset({"chrome"}),
        previous_target_ids=frozenset({"chrome", "jetbrains"}),
    )
    assert enabled == frozenset({"chrome", "jetbrains"})
...
def test_execute_oserror(tmp_path: Path, service: SpellSyncService, mock_targets) -> None:
    ...
    with patch(
        ...
        side_effect=OSError(28, "No space left on device", "/secret/spell-sync.toml"),
    ):
        execution = service.execute_target_settings_update(
            prepared,
            confirmed_update_id=prepared.update_id,
    ...
    assert execution.outcome is TargetSettingsOutcome.FAILED
    assert "could not write spell-sync.toml" in execution.message
    assert "/secret/" not in execution.message
...
def test_build_target_settings_reports() -> None:
    from spell_sync.application.operation_reports import build_target_settings_operation_report
    from spell_sync.project_setup.target_settings import (
        PreparedTargetSettingsUpdate,
