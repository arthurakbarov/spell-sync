...
class TestRuntimeHelpers(unittest.TestCase):
    ...
    def test_plan_removals_empty_wordlist_aborts(self):
        with tempfile.TemporaryDirectory() as d:
            wordlist = os.path.join(d, "wordlist.txt")
...
class TestRuntimeHelperCoverageGaps(unittest.TestCase):
...
<application><component name="CachedDictionaryState"></component></application>"""
        words, _component, parsed = io_mod._jetbrains_words_from_xml(xml)
...
class TestPlanRemovalsHuman(unittest.TestCase):
    ...
    def test_print_removals_helper(self):
        ...
        removal_mod.print_removals(removal_mod.list_removals_from_preview(preview))
