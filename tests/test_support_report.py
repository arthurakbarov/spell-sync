...
from spell_sync.support.path_redaction import redact_path
...
def test_support_report_unreadable_wordlist_is_unknown(project_env) -> None:
    service, _opts, _project, wordlist, _home = project_env
    ...
    assert any(notice.code == "unreadable_wordlist" for notice in report.notices)
    text = format_support_report_text(report)
    assert "unknown" in text
    assert "Word list unreadable" in text


def test_support_report_cleanup_pending_notice(
    project_env, monkeypatch: pytest.MonkeyPatch
) -> None:
    ...
    monkeypatch.setattr(
        ...
        lambda _path, **_kwargs: JournalLoadResult(JournalLoadStatus.VALID_COMPLETED, None),
    )
    report = service.load_support_report(
        SupportReportRequest(project=ProjectRef(wordlist=wordlist)),
    )
    assert report.project.pending_recovery is False
    assert report.recovery.pending_recovery is False
    assert report.recovery.journal_status == JournalLoadStatus.VALID_COMPLETED.value
    assert any(notice.code == "cleanup_pending" for notice in report.notices)
    assert "Recovery files remain" in format_support_report_text(report)
...
def test_support_report_path_redaction(project_env) -> None:
    service, _opts, project, wordlist, home = project_env
    report = service.load_support_report(
        SupportReportRequest(project=ProjectRef(wordlist=wordlist)),
    )
...
def test_export_support_report_write_failure(project_env, tmp_path: Path, monkeypatch) -> None:
    ...
    monkeypatch.setattr("spell_sync.io.os.open", fail_open)
...
def test_cli_support_report_stdout(project_env, monkeypatch: pytest.MonkeyPatch) -> None:
    service, opts, _project, _wordlist, _home = project_env
    load_report = service.load_support_report
    monkeypatch.setattr(
        "spell_sync.application.service.SpellSyncService.load_support_report",
        lambda self, request: load_report(request),
    )
