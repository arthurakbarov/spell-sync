...
def _completed_pull_report() -> OperationReport:
    return OperationReport(
        ...
        summary="Collect my words completed.",
    )
...
def _completed_push_report() -> OperationReport:
    return OperationReport(
        ...
        summary="Update my apps completed.",
        target_updates=(
            TargetUpdateReport(
                name="editors",
                additions=1,
...
def test_session_export_cleanup_pending_is_not_recovery_required() -> None:
    from spell_sync.application.product_concepts import RECOVERY_CLEANUP_REMAINING

    session = ReviewSession(pull_skipped=True, push_skipped=True)
    export = build_session_report_export(session, cleanup_pending=True)
    assert export.recovery_required is False
    assert export.recovery_note == RECOVERY_CLEANUP_REMAINING
