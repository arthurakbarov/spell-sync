...
class TestAdversarialRegressions(unittest.TestCase):
    ...
    def test_r2_parent_swap_before_final_open(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def hook(boundary: str, name: str) -> None:
                if boundary == "before_file_open" and name == "artifact":
                    real = safe
    ...
    def test_r3_snapshot_path_swap_victim_unchanged(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def hook(boundary: str, name: str) -> None:
                if boundary != "before_snapshot_copy":
                    return
    ...
    def test_r4_cleanup_directory_swap_preserves_unrelated(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            def hook(boundary: str, _name: str) -> None:
                if boundary != "before_cleanup_listdir":
                    return
    ...
    def test_r5_unsafe_internal_reads(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertNotIn(str(external_journal), result.detail or "")
            self.assertNotIn("/", result.detail or "")
