    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Import):
            for alias in node.names:
                hits.extend(
                    f"{module_name}: import {alias.name}" for token in banned if token in alias.name
                )
        if isinstance(node, ast.ImportFrom) and node.module:
            hits.extend(
                f"{module_name}: from {node.module}" for token in banned if token in node.module
            )
    return hits
...
class TestApplicationRequests(unittest.TestCase):
    ...
    def test_cli_commands_have_request_mappers_or_are_presentation_only(self) -> None:
        ...
        cli_utilities = {"add", "config-check", "lint", "git-save"}
        adapters = {"ui"}
    ...
    def test_project_scope_helpers_cover_json_and_early_returns(self) -> None:
        ...
        with patch(
            ...
            from spell_sync.application.mutation_scope import mutation_scope_for

            gen = mutation_scope_for(
