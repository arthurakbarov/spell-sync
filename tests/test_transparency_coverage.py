"""Coverage tests for Spell Sync 0.2.1 transparency features."""
...
def test_support_report_export_text_and_errors(tmp_path: Path) -> None:
    report = SupportReport(
        ...
        architecture="arm64",
        installation=InstallationInfo("0.2.1", "test"),
...
def test_support_report_cmd_json_output(tmp_path: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setattr(
        ...
        lambda *_a, **_k: MagicMock(
            ...
            operating_system="test",
            architecture="arm64",
            installation=MagicMock(package_version="0.2.1", installation_type="test"),
