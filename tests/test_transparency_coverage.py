...
def test_target_details_filtering_and_states() -> None:
    ...
    assert "full word list" in latin.filtering_label.lower()
...
def test_doctor_export_worker_branches() -> None:
    ...
    async def _run() -> None:
        ...
        async with app.run_test(size=(100, 32)) as pilot:
            ...
            screen._export_worker_handle = success
            screen._finish_export_worker(success)

            error = MagicMock(state=WorkerState.ERROR)
            screen._export_in_progress = True
            screen._export_worker_handle = error
            screen._finish_export_worker(error)

            pending = MagicMock(state=WorkerState.PENDING)
            screen._export_in_progress = True
            screen._export_worker_handle = pending
            screen._finish_export_worker(pending)
            ...
            bad = MagicMock(state=WorkerState.SUCCESS, result="bad")
            screen._export_in_progress = True
            screen._export_token = 1
            screen._export_worker_handle = bad
            screen._finish_export_worker(bad)
            ...
            screen._export_in_progress = True
            screen._export_token = 1
            screen._export_worker_handle = collision
...
def test_review_export_worker_branches() -> None:
    ...
    async def _run() -> None:
        ...
        async with app.run_test(size=(100, 32)) as pilot:
            ...
            cancelled = MagicMock(state=WorkerState.CANCELLED)
            screen._export_in_progress = True
            screen._export_token = 1
            screen._export_started_token = 1
            screen._export_worker_handle = cancelled
            screen._finish_export_worker(cancelled)
