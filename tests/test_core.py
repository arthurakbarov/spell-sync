...
class TestWords(unittest.TestCase):
    ...
    def test_normalize_token_nfc_collapses_decomposed(self):
        import unicodedata
        ...
        self.assertEqual(words_mod.normalize_token(nfd), nfc)
        self.assertEqual(
            words_mod.merge_case_duplicates([nfc, nfd]),
            [nfc],
        )
...
class TestIO(unittest.TestCase):
    ...
    def test_json_and_chrome_roundtrip(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(
                Dictionary("sublime", jp, DictionaryFormat.JSON).read(quiet=True), {"os"}
            )
...
class TestLint(unittest.TestCase):
    def test_report_and_whitelist(self):
        ...
        self.assertNotIn("very_short", rep)
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
        ...
        # Length is not a quality signal — short intentional tokens stay soft-clean.
        self.assertNotIn("very_short", clean)
