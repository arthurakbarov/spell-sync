...
class TestCommands(unittest.TestCase):
    ...
    def test_cmd_status_includes_optional_app_honesty(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            wordlist = os.path.join(d, "wordlist.txt")
            path_a = os.path.join(d, "a.txt")
            path_b = os.path.join(d, "b.txt")
            ...
            with patch_commands_service(load_status=snapshot):
                buf = io.StringIO()
                with redirect_stdout(buf):
                    code = commands.cmd_status(CliOptions(wordlist=wordlist))
...
class TestPushServiceGate(unittest.TestCase):
    ...
    def test_cmd_status_quiet_log_no_session_outcome(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch_commands_service(load_status=status_snapshot_from_run(run)):
                cli_log.quiet = True
                try:
                    code = commands.cmd_status(CliOptions(wordlist=str(wordlist)))
                finally:
                    cli_log.quiet = False
            self.assertEqual(code, int(ExitCode.OK))
    ...
    def test_cmd_init_unreadable_wordlist_fails_closed(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            wordlist = Path(d) / "wordlist.txt"
            wordlist.write_bytes(b"good\n\x00bad\n")
            code = commands.cmd_init(CliOptions(wordlist=str(wordlist)))
            self.assertEqual(code, int(ExitCode.PUSH_ABORT))
            self.assertFalse((Path(d) / "spell-sync.toml").exists())
...
class TestRecoverCmdCoverage(unittest.TestCase):
    ...
    def test_recover_discards_inert_unfinished_update(self) -> None:
        with tempfile.TemporaryDirectory() as d:
            wordlist = Path(d) / "wordlist.txt"
            wordlist.write_text("a\n", encoding="utf-8")
            preview = replace(
            ...
            discarded = RecoveryExecution(
                preview=preview,
                result=ExitCode.OK,
                outcome=RecoveryOutcome.DISCARDED,
                message="",
            )
            discard = MagicMock(return_value=discarded)
            with patch_recover_service(
                inspect_recovery=preview,
                execute_recovery_discard=discard,
                build_recovery_report=MagicMock(),
            ):
                buf = io.StringIO()
                with redirect_stdout(buf):
            ...
            self.assertIn("discarded", buf.getvalue().lower())
