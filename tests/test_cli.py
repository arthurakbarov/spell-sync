...
class TestMain(unittest.TestCase):
    ...
    def test_main_json_emission_resets_between_invocations(self):
        with redirect_stdout(io.StringIO()) as first:
            code1 = cli_mod.main(["spell-sync", "version", "--json"])
        with redirect_stdout(io.StringIO()) as second:
            code2 = cli_mod.main(["spell-sync", "version", "--json"])
        self.assertEqual(code1, 0)
        self.assertEqual(code2, 0)
        self.assertIn('"command": "version"', first.getvalue())
        self.assertIn('"command": "version"', second.getvalue())
...
class TestSubprocessSmoke(unittest.TestCase):
    ...
    def test_status_json_stdout(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            proc = _run_sync("status", "--json", "--wordlist", str(wordlist))
            self.assertEqual(proc.returncode, 0)
            data = json.loads(proc.stdout)
    ...
    def test_add_subprocess_appends_words(self):
        with tempfile.TemporaryDirectory() as d:
            env = os.environ.copy()
            ...
            proc = subprocess.run(
                [sys.executable, "-m", "spell_sync", "add", "AcmeCorp", "--json"],
                cwd=d,
                capture_output=True,
                text=True,
                env=env,
            ...
            self.assertIn("AcmeCorp", data["added"])
            self.assertIn("AcmeCorp", (Path(d) / "wordlist.txt").read_text(encoding="utf-8"))

    def test_add_json_rejects_junk_and_missing_wordlist(self):
        with tempfile.TemporaryDirectory() as d:
            env = os.environ.copy()
            env["PYTHONPATH"] = str(_ROOT)
            missing_path = str(Path(d) / "missing" / "wordlist.txt")
            ...
            self.assertEqual(missing_payload["command"], "add")
            ...
            self.assertEqual(init.returncode, 0, msg=init.stderr)
            junk = subprocess.run(
                [sys.executable, "-m", "spell_sync", "add", "!!!", "--json"],
                cwd=d,
                capture_output=True,
                text=True,
                env=env,
            )
            self.assertEqual(junk.returncode, int(ExitCode.LINT_FAILED))
            junk_payload = json.loads(junk.stdout)
            self.assertEqual(junk_payload["rejected"], ["!!!"])
            self.assertEqual(junk_payload["added"], [])
