...
class TestDoctor(unittest.TestCase):
    ...
    def test_doctor_missing_wordlist_error(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIsNone(report.wordlist_count)
    ...
    def test_doctor_empty_wordlist_warn(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(
                any(check.message.startswith("word list is empty") for check in report.checks)
            )
            add_actions = [action for action in report.actions if action.id == "add-words"]
            self.assertEqual(len(add_actions), 1)
            self.assertEqual(add_actions[0].command, "spell-sync add WORD")
    ...
    def test_doctor_check_exits_on_errors(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("word list is unreadable", err.getvalue())
    ...
    def test_doctor_chrome_probe_none_warns(self):
        with tempfile.TemporaryDirectory() as d:
            wordlist = os.path.join(d, "wordlist.txt")
            write_text_words(wordlist, ["alpha"], "utf-8", False, quiet=True)
            run = make_sync_run(wordlist, dictionaries=[])
            with (
                patch.object(report_mod, "chrome_dictionaries_enabled", return_value=True),
                patch.object(report_mod, "is_chrome_running", return_value=None),
            ):
                report = doctor_mod.build_doctor_report(run)
            self.assertTrue(
                any(
                    "Could not verify whether Chrome is quit" in c.message
    ...
    def test_doctor_pre_push_stale_warn(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            pre_push.write_text(
                ...
                '. "$root/scripts/lib/personal-paths.sh"\n',
                encoding="utf-8",
            )
            pre_push.chmod(0o755)
    ...
    def test_doctor_pre_push_current_template_not_stale(self):
        with tempfile.TemporaryDirectory() as d:
            root = Path(d)
            subprocess.run(["git", "init"], cwd=str(root), check=True, capture_output=True)
            wordlist = str(root / "wordlist.txt")
            ...
            pre_push = hooks_dir / "pre-push"
            pre_push.write_text(
                'guard_remote="${SPELL_SYNC_GUARD_REMOTE:-$(git config --get spell-sync.guard-remote 2>/dev/null || true)}"\n'
                'paths="$(spell_sync_personal_paths)" || exit 1\n',
                encoding="utf-8",
            )
            pre_push.chmod(0o755)
            pre_commit = hooks_dir / "pre-commit"
            pre_commit.write_text(
                "#!/bin/sh\n# pre-commit: lint staged wordlist.txt blob(s)\n"
                'git show ":$path" >/dev/null\n',
                encoding="utf-8",
            )
            pre_commit.chmod(0o755)
            report = doctor_mod.build_doctor_report(run)
            ...
            self.assertFalse(
                any("pre-commit hook is outdated" in c.message for c in report.checks),
            )
            self.assertTrue(report.git_hooks is not None)
            self.assertFalse(report.git_hooks.pre_push_stale)
            self.assertFalse(report.git_hooks.pre_commit_stale)
    ...
    def test_doctor_unreadable_wordlist_error(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertTrue(any("word list is unreadable" in msg for msg in error_msgs))
