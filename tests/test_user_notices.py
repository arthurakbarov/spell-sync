...
from spell_sync.application.dashboard_builders import build_dashboard_issues
...
class TestUserNoticeCatalog(unittest.TestCase):
    ...
    def test_skip_reason_maps_to_application_running(self):
        ...
        self.assertIn("app was running", format_skip_status("Google Chrome is running"))
...
class TestPlannedActualReports(unittest.TestCase):
    ...
    def test_push_planned_actual_format(self):
        ...
        self.assertIn("app was running", text)
    ...
    def test_pull_planned_actual_with_skipped_source(self):
        ...
        self.assertRegex(text, r"Actual additions:\s+15")
        self.assertRegex(text, r"Skipped sources:\s+1")
        self.assertIn("app custom dictionary could not be read", text)
        _assert_notice_safe(text)

    def test_pull_planned_actual_uses_source_skip_reason(self):
        preview = PullPreview(
            wordlist_path="/tmp/wordlist.txt",
            additions=2,
            before_count=1,
        ...
        text = "\n".join(format_pull_planned_actual_lines(preview, execution))
        self.assertIn("corrupt or unsupported", text)
    ...
    def test_pull_operation_report_uses_planned_actual(self):
        ...
        self.assertRegex(text, r"Actual additions:\s+3")
...
class TestUserNoticeCoverage(unittest.TestCase):
    ...
    def test_skip_reason_branches(self):
        ...
        self.assertEqual(skip_reason_to_notice_code("unreadable"), "target_unreadable")
        self.assertEqual(skip_reason_to_notice_code("file unreadable"), "target_unreadable")
        self.assertEqual(skip_reason_to_notice_code("backup_failed"), "backup_failed")
        self.assertEqual(skip_reason_to_notice_code("blocked_by_user"), "update_blocked")
        self.assertEqual(skip_reason_to_notice_code("blocked by policy"), "update_blocked")
        self.assertEqual(format_skip_status("unreadable"), "Skipped — unreadable")
        self.assertEqual(format_skip_status("blocked_by_user"), "Skipped — not selected")
        self.assertEqual(format_skip_status("backup_failed"), "Skipped — backup failed")
        self.assertEqual(skip_reason_to_notice_code("skipped"), "target_skipped")
        self.assertEqual(format_skip_status("skipped"), "Skipped")
        self.assertEqual(skip_reason_to_notice_code("unknown-reason"), "target_skipped")
        self.assertEqual(format_skip_status("unknown-reason"), "Skipped")
