...
class TestNeovimMkspell(unittest.TestCase):
    ...
    def test_mkspell_after_neovim_writes_disabled(self):
        with patch.object(mkspell_mod, "run_mkspell_for_add_file") as run:
            mkspell_mod.mkspell_after_neovim_writes(("nvim",), settings=_SETTINGS)
...
class TestMkspellAfterPush(unittest.TestCase):
    ...
    def test_mkspell_after_neovim_writes_runs_for_nvim_dict(self):
        add_path = Path("/tmp/en.utf-8.add")
        with (
            patch(
                "spell_sync.paths.neovim_dict_paths",
                return_value=[("nvim", add_path)],
            ),
            patch.object(mkspell_mod, "run_mkspell_for_add_file", return_value=True) as mkspell,
        ):
            mkspell_mod.mkspell_after_neovim_writes(("nvim",), settings=_MKSPELL_ENABLED)
        mkspell.assert_called_once_with(add_path)

    def test_mkspell_after_neovim_writes_accepts_nvim_prefixed_names(self):
        add_path = Path("/tmp/en.utf-8.add")
        with (
            patch(
                "spell_sync.paths.neovim_dict_paths",
    ...
    def test_run_mkspell_missing_add_file(self):
        missing = Path("/missing.add")
        buf = io.StringIO()
        with (
            patch.object(mkspell_mod.shutil, "which", return_value="/usr/bin/nvim"),
            redirect_stdout(buf),
        ):
            self.assertFalse(mkspell_mod.run_mkspell_for_add_file(missing))
        out = buf.getvalue()
        self.assertIn("missing.add", out)
        self.assertNotIn("/missing.add", out)
    ...
    def test_run_mkspell_subprocess_oserror(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with (
                ...
                out = buf.getvalue()
                self.assertIn("mkspell failed", out)
                self.assertIn(add_path.name, out)
                self.assertNotIn(str(add_path), out)
