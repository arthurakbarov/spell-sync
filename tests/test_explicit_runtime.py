...
class TestExplicitRuntimeCoverage(unittest.TestCase):
    def test_config_push_helpers_read_runtime_settings(self) -> None:
        from spell_sync.config import push_guard_local_min

        settings = RuntimeSettings.defaults()
        self.assertIsInstance(push_guard_local_min(settings=settings), int)
        self.assertIsInstance(settings.push.strict, bool)

    def test_support_report_missing_wordlist_is_unknown(self) -> None:
        ...
        with tempfile.TemporaryDirectory() as tmp:
            ...
            run = make_sync_run(wordlist=wordlist)
            service = type(
                "Service",
                (),
                {
                    "load_operation_history": lambda self, limit=5: OperationHistorySnapshot(
                        records=()
                    )
                },
            )()
        ...
        self.assertFalse(any(notice.code == "unreadable_wordlist" for notice in report.notices))
