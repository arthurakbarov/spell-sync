...
def test_parse_technical_log_line_returns_none_for_plain_text() -> None:
...
def test_format_log_line_for_display_handles_structured_and_plain_lines() -> None:
    ...
    plain = "removed words: [secret-token-value]"
    sanitized = format_log_line_for_display(plain)
...
def test_read_technical_log_tail_handles_mixed_rotation_backup(tmp_path) -> None:
    ...
    paths = resolve_app_state_paths(state_root=tmp_path / "state")
    configure_file_logging(paths)
    plain = "2026-07-21 INFO removed words: sentinel-value"
    structured = serialize_technical_event(_sample_event())
    ...
    paths.technical_log.write_text(
        "\n".join((plain, structured, malformed)) + "\n",
        encoding="utf-8",
    )
    backup = paths.technical_log.with_suffix(paths.technical_log.suffix + ".1")
    backup.write_text("rotated plain line\n", encoding="utf-8")
