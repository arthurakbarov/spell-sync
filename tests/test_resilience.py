...
class TestLintResilience(unittest.TestCase):
    def test_missing_wordlist_returns_unreadable(self):
        ...
        self.assertIn("word list is unreadable", buf.getvalue())
        self.assertNotIn("/nonexistent", buf.getvalue())
...
class TestIoResilience(unittest.TestCase):
    ...
    def test_atomic_write_uses_unique_temp_suffix(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            # First write: payload temp only. Second: bak partial + payload temp.
            self.assertEqual(len(temps), 3)
            self.assertTrue(any(p.suffix == ".tmp" for p in temps))
...
class TestIoHelpers(unittest.TestCase):
    ...
    def test_atomic_write_backup_failure_aborts_write(self):
        with tempfile.TemporaryDirectory() as d:
            target = Path(d) / "dict.txt"
            target.write_text("old\n", encoding="utf-8")
            ...
            self.assertIn("backup not created", message)
            self.assertNotIn(str(target), message)
            self.assertIn(target.name, str(raised.exception))
    ...
    def test_rotate_backup_chain_shifts_numbered_files(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            Path(d, "dict.txt.1.bak").write_text("gen2\n", encoding="utf-8")
            io_mod.rotate_backup_chain(base, keep=3)
            self.assertFalse(base.exists())
            self.assertEqual(Path(d, "dict.txt.1.bak").read_text(encoding="utf-8"), "gen1\n")
            self.assertEqual(Path(d, "dict.txt.2.bak").read_text(encoding="utf-8"), "gen2\n")
    ...
    def test_atomic_write_rotates_before_backup(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertEqual(Path(d, "dict.txt.1.bak").read_text(encoding="utf-8"), "previous\n")
...
class TestIoFormats(unittest.TestCase):
    ...
    def test_write_json_overwrites_corrupt_existing(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            self.assertIn("new", json.loads(path.read_text(encoding="utf-8"))["added_words"])
