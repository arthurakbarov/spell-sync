...
class TestPushStrict(unittest.TestCase):
    ...
    def test_strict_aborts_on_partial_backup_failure(self):
        with tempfile.TemporaryDirectory() as d:
            ...
            with patch(
                "spell_sync.push_transaction.copy_trusted_snapshot_file", selective_snapshot
            ):
                buf = io.StringIO()
                with redirect_stdout(buf):
                    result = run.push_from_wordlist()
    ...
    def test_finish_push_partial_summary_line(self):
        ...
        out = buf.getvalue()
        self.assertIn("partial push", out)
        self.assertNotIn("exit 5", out)
...
class TestRemovalWarning(unittest.TestCase):
    def test_confirm_push_removals_prompts_when_over_limit(self):
        ...
        prepared = MagicMock()
        prepared.max_removals.return_value = 60
        prepared.ctx.settings = RuntimeSettings.defaults()
        preview = sample_preview(prepared=prepared, removals=60)
        ...
        with (
            ...
            self.assertFalse(
                command_helpers.confirm_push_removals_for_preview(preview, DEFAULT_OPTS)
            )
    ...
    def test_confirm_push_removals_yes_skips_prompt(self):
        ...
        with patch(limit_patch, return_value=50):
            self.assertTrue(
                command_helpers.confirm_push_removals_for_preview(preview, CliOptions(yes=True))
            )
