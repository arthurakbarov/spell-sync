...
MANUAL_SCOPE = frozenset({"read-only", "mutation", "mutation-ui"})
...
def validate_target_validation_payload(payload: dict[str, Any]) -> list[str]:
    ...
    for index, item in enumerate(targets):
        ...
        if scope is not None and scope not in MANUAL_SCOPE:
            errors.append(f"{prefix}:manual_scope:enum")
        if item.get("manual_validation") == "pass" and scope is None:
            errors.append(f"{prefix}:manual-pass:requires-manual_scope")
        if item.get("manual_validation") != "pass" and scope is not None:
            errors.append(f"{prefix}:manual_scope:null-unless-pass")
