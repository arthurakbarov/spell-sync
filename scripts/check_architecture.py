...
CHECK_ID = "ARCH"
# Optional map path via --project-map (used from private maintainer scripts).
PROJECT_MAP_START = "[project-map:start]"
PROJECT_MAP_END = "[project-map:end]"
TEST_GROUPS_PATH = ROOT / "tests" / "test-groups.toml"
...
def _format_violation(violation: ArchitectureViolation) -> str:
    if violation.path is None:
        ...
        try:
            location = str(violation.path.relative_to(ROOT))
        except ValueError:
...
def _application_parent_package_import_hits(source_path: Path) -> list[str]:
    """Imports that reach application via the parent package name, not a sibling."""
    tree = ast.parse(source_path.read_text(encoding="utf-8"))
    hits: list[str] = []
    for node in ast.walk(tree):
        ...
        elif module == "spell_sync.application" or module.startswith("spell_sync.application."):
...
def _check_application_imports() -> list[ArchitectureViolation]:
    ...
    for path in sorted(app_root.rglob("*.py")):
        for hit in _application_parent_package_import_hits(path):
            violations.append(
                ArchitectureViolation(
                    f"{CHECK_ID}-APP-002",
                    path,
                    hit,
                    "import application siblings relatively, not via the parent package name",
                )
            )
    return violations
...
def _generated_project_map_section() -> str:
    ...
    lines = [
        (
            "_Generated by `scripts/check_architecture.py` from `tests/test-groups.toml`; "
    ...
    lines.append("")
    lines.append(
        "Run the public gate via GitHub Actions (`.github/workflows/ci.yml`) or locally "
        "with `uv run pytest tests -q` after `uv sync --locked --all-groups`."
    )
    lines.append("")
    return "\n".join(lines)
...
def _check_project_map(
    *,
    write: bool,
    project_map: Path | None,
) -> list[ArchitectureViolation]:
    violations: list[ArchitectureViolation] = []
    if project_map is None or not project_map.is_file():
        return []
        ...
        if write:
            ...
            violations.append(
                ArchitectureViolation(
                    ...
                    "run scripts/check_architecture.py --write --project-map <path>",
                )
            )
    return violations
...
def collect_violations(
    *,
    ...
    violations.extend(_check_project_map(write=write_project_map, project_map=project_map))
    return violations
...
def main(argv: list[str] | None = None) -> int:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="Regenerate the project map generated section when --project-map is set.",
    )
    parser.add_argument(
        "--project-map",
        type=Path,
        default=None,
        help="Optional project-map markdown to validate or update.",
    )
