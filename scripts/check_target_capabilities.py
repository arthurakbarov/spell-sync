...
def _render_matrix(data: dict[str, object]) -> str:
    ...
    for item in sorted(entries, key=lambda row: (row["target_id"], row["platform"])):
        ...
        scope = item.get("manual_scope")
        if manual == "pass" and isinstance(scope, str) and scope.strip():
            manual_cell = f"{manual} ({scope})"
        else:
            manual_cell = str(manual)
        tested_on = item.get("tested_on") or "—"
        lines.append(
            f"{capability.display_name} | {item['platform']} | {pull} | {push} | "
            f"{filtering} | {profiles} | {close} | {auto} | {manual_cell} | {tested_on}"
        )
    return "\n".join(lines)
...
def marker_structure_errors(text: str) -> list[str]:
    errors: list[str] = []
    if OBSOLETE_MATRIX_MARKER in text:
        errors.append("SUPPORTED_APPS.md: obsolete target-capabilities-matrix marker present")
    if OBSOLETE_BRACKET_START in text or OBSOLETE_BRACKET_END in text:
        errors.append("SUPPORTED_APPS.md: obsolete bracket target-capabilities markers present")
    start_count = text.count(START_MARKER)
    end_count = text.count(END_MARKER)
    if start_count != 1:
        errors.append(
            f"SUPPORTED_APPS.md: start marker must appear exactly once (found {start_count})"
        )
    if end_count != 1:
        errors.append(f"SUPPORTED_APPS.md: end marker must appear exactly once (found {end_count})")
    if start_count != 1 or end_count != 1:
        return errors
    start = text.index(START_MARKER)
    end = text.index(END_MARKER)
    if end <= start:
        errors.append("SUPPORTED_APPS.md: end marker must appear after start marker")
    return errors
...
def _check_supported_doc(data: dict[str, object]) -> list[str]:
    if not SUPPORTED_DOC.is_file():
        return ["missing docs/SUPPORTED_APPS.md"]
    ...
    if actual != expected:
        return ["SUPPORTED_APPS.md generated matrix is stale — run --write"]
    return []
...
def main() -> int:
    ...
    parser.add_argument(
        "--write",
        action="store_true",
        help="regenerate SUPPORTED_APPS.md matrix and bundled validation JSON",
    )
