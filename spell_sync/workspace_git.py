...
def _tracked_pathspecs(repo: Path, project_dir: Path) -> tuple[str, ...] | None:
    ...
    try:
        ...
        return None
    return tuple(
        name if rel == Path(".") else (rel / name).as_posix() for name in sorted(_TRACKED_NAMES)
    )


def inspect_workspace_git(start: Path) -> WorkspaceGitStatus | None:
    """Inspect dirty personal files for a project directory or word-list path.

    ``start`` may be the project folder or ``wordlist.txt`` (including a path
    that does not exist yet). Returns None when Git is unavailable or the
    folder is not a repository.
    """
    project_dir = start if start.is_dir() else start.parent
    ...
    if specs is None:
        return None
    try:
        completed = _git(repo, "status", "--porcelain=v1", "-z", "--", *specs)
    except OSError, subprocess.TimeoutExpired:
        return None
    if completed.returncode != 0:
        return None
    ...
    while index < len(entries):
        ...
        if len(entry) < 3:
            continue
        # Porcelain -z: "XY path" or rename/copy "XY old" then next entry is new path.
        path = entry[3:]
        if (entry[0] in "RTC" or entry[1] in "RTC") and index < len(entries) and entries[index]:
            path = entries[index]
            index += 1
        name = Path(path).name
