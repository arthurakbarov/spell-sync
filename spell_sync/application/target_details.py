...
def _filtering_label(filter_kind: TargetFilterKind) -> str:
    if filter_kind == TargetFilterKind.FULL:
        return "Full canonical personal wordlist"
...
def _close_policy_label(capability: TargetCapability) -> str:
    if capability.close_policy.value == "block-if-running":
        return f"Push is blocked while {capability.display_name} is running"
...
def format_target_details_text(details: TargetDetailsSnapshot) -> str:
    ...
    lines = [
        ...
        "",
        "State",
        f"  Enabled: {'Yes' if details.enabled else 'No'}",
        f"  Detected: {'Yes' if details.detected else 'No'}",
        f"  Readable: {'Yes' if details.readable else 'No'}",
        f"  Writable: {'Yes' if details.writable else 'No'}",
        f"  Status: {details.runtime_state}",
        "",
        "Capabilities",
        f"  Pull: {'Supported' if details.pull_supported else 'Not supported'}",
        f"  Push: {'Supported' if details.push_supported else 'Not supported'}",
        f"  Filtering: {details.filtering_label}",
        f"  Profiles: {details.profile_model}",
        f"  Running application: {details.close_policy_label}",
        f"  Recovery protection: {'Enabled' if details.recovery_protected else 'Disabled'}",
        "",
        "Discovery",
        f"  Source: {details.discovery_source}",
    ]
    if details.custom_dictionary_path:
        lines.append(f"  Custom dictionary: {details.custom_dictionary_path}")
    lines.extend(
        [
            "",
            "Validation",
            f"  Automated fixtures: {details.automated_validation}",
            f"  Real application test: {details.manual_validation}",
        ]
    )
