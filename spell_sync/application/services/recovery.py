...
from ...guest_messages import RECOVERY_DISCARD_FAILED, RECOVERY_DISCARDED_MESSAGE
...
from ..recovery_preview_builders import build_recovery_preview
        ...
        *,
        ...
        with self._ctx.runtime.mutation_scope(
            ...
            if preview.status in {
                ...
                if journal_result.status not in allowed_statuses:
                    return RecoveryExecution(
                        preview=preview,
                        result=ExitCode.PUSH_ABORT,
                        outcome=RecoveryOutcome.FAILED,
                ...
                if not journal_ok:
                    return RecoveryExecution(
                        ...
                        result=ExitCode.PUSH_ABORT,
                        outcome=RecoveryOutcome.FAILED,
                        message=detail or RECOVERY_DISCARD_FAILED,
                    )
            elif (
                preview.status is RecoveryStatus.RECOVERABLE
                and not preview.can_recover
                and preview.recoverable_count == 0
                and preview.conflict_count == 0
                and preview.failure_count == 0
            ):
                # Inert unfinished journal (no writes started). Unlock via metadata discard.
                if (
                    journal_result.status is not JournalLoadStatus.VALID_IN_PROGRESS
                    or journal_result.journal is None
                ):
                    return RecoveryExecution(
                        preview=preview,
                        result=ExitCode.PUSH_ABORT,
                        outcome=RecoveryOutcome.FAILED,
                        message="Unfinished journal is no longer present as reviewed.",
                    )
                if journal_result.content_digest != preview.preview_fingerprint:
                    return RecoveryExecution(
                        preview=preview,
                        result=ExitCode.PUSH_ABORT,
                        outcome=RecoveryOutcome.FAILED,
                        message="Unfinished journal changed after preview.",
                    )
                live_plans = _operation_deps.plan_recovery_from_journal(journal_result.journal)
                if any(
                    plan.write_started or plan.write_completed or plan.status == "ready"
                    ...
                    return RecoveryExecution(
                        ...
                        result=ExitCode.PUSH_ABORT,
                        outcome=RecoveryOutcome.FAILED,
                        message="Journal is no longer inert; use Recover instead of Discard.",
                    )
                discard_result = _operation_deps.cleanup_after_successful_recovery(
                ...
                if not discard_result.ok:
                    return RecoveryExecution(
                        ...
                        result=ExitCode.PUSH_ABORT,
                        outcome=RecoveryOutcome.FAILED,
                        message=discard_result.detail or RECOVERY_DISCARD_FAILED,
                    )
            else:
                if (
