...
from ...guest_messages import PULL_WORDLIST_WRITE_FAILED
...
from ...sync_run import SyncRun, dictionary_diffs_from_prepared, sync_run_for
...
from ..project_resolution import effective_push_strict
from ..push_pull_preview_builders import build_pull_add_from_preview, build_push_preview
from ..reports import OperationOutcome, PullExecution, PullPreview, PushExecution, PushPreview
from ..requests import PullRequest, PushRequest, RecoveryRequest, StatusRequest
from ._shared import (
...
class SyncService:
    ...
    def load_extra_word_inventory(self, request: StatusRequest) -> ExtraWordInventory:
        ...
        return build_extra_word_inventory(run)
    ...
    def subtract_extra_words(
        ...
        with self._ctx.runtime.mutation_scope(
            ...
            return subtract_extra_words_from_sources(run, inventory, words)

    def load_push_preview(self, request: PushRequest) -> PushPreview:
        resolved = self._ctx.runtime.resolve_read(request.project)
        run = sync_run_for(
            resolved,
            strict_push=effective_push_strict(request, settings=resolved.context.settings),
        )
    ...
    def load_push_plan(
        ...
        *,
        ...
        diffs = tuple(dictionary_diffs_from_prepared(prepared, verbose=verbose))
        result = SyncRun(context=prepared.ctx).plan_push(prepared=prepared)
        ...
        with self._ctx.runtime.mutation_scope(
            ...
            if isinstance(result, ExitCode):
                ...
                if not conflict:
                    ...
                    conflict = verify_pull_source_fingerprints(preview.source_rows) is not None
                emit_technical(
                    emitter,
                    build_technical_event(
                        event_id=(
                ...
                return PullExecution(
                    ...
                    message=(
                        "Word list or a pull source changed after the preview was created."
                        if conflict
                        else PULL_WORDLIST_WRITE_FAILED
                    ),
                    warnings=preview.warnings,
                )
            ...
            emit_technical(
                ...
                build_technical_event(
                    ...
                    category=EventCategory.LIFECYCLE,
                    severity=(
                        EventSeverity.WARNING if preview.sources_skipped else EventSeverity.SUCCESS
                    ),
                    correlation_id=correlation_id,
                    outcome=(
                        OperationOutcome.COMPLETED_WITH_WARNINGS
                        if preview.sources_skipped
            ...
            message = format_pull_word_count_line(
                before,
                after,
                skipped_sources=len(preview.sources_skipped),
            )
            return PullExecution(
                preview=preview,
                result=(before, after),
    ...
    def pull_execution_from_result(
        ...
        return PullExecution(
            ...
            result=(before, after),
            outcome=OperationOutcome.COMPLETED,
            message=format_pull_word_count_line(before, after),
            warnings=preview.warnings,
        )
    ...
    def push_execution_from_result(
        ...
        return PushExecution(
            ...
            result=result,
            outcome=outcome,
            message="Update my apps completed.",
            target_updates=updates,
            push_preview=preview,
        )
