...
def build_dashboard_issues(
    ...
    *,
    ...
    if snapshot.empty_wordlist and snapshot.wordlist_error is None:
        issues.append(
            DashboardIssue(
                ...
                suggested_action="Add words, or run Collect my words.",
            )
        )
    ...
    if enable_sublime(settings=validated.context.settings):
...
def build_status_detail_snapshot(run: SyncRun) -> StatusDetailSnapshot:
    ...
    skipped_names = set(snapshot.skipped_unreadable) | set(snapshot.skipped_corrupt)
    ...
    if snapshot.empty_wordlist:
        warnings.append(
            "wordlist is empty — push will abort; dictionaries won't change.",
        )
    if enable_sublime(settings=run.context.settings):
        override = user_added_words_override_message()
        if override is not None:
            warnings.append(override)
    git_status = inspect_workspace_git(Path(run.wordlist_str).parent)
    if git_status is not None and git_status.is_dirty:
...
def _project_honesty_warnings(wordlist: Path, settings) -> list[str]:
...
def build_push_preview(
    ...
    *,
    ...
    warnings.extend(
        _project_honesty_warnings(
            Path(prepared.ctx.wordlist_str),
            prepared.ctx.settings,
        )
    )
...
def build_pull_preview(run: SyncRun) -> PullPreview:
    ...
    warnings.extend(_project_honesty_warnings(Path(wordlist_path), run.context.settings))
    return PullPreview(
        wordlist_path=wordlist_path,
...
def build_push_operation_report(execution: PushExecution) -> OperationReport:
    ...
    if outcome is OperationOutcome.COMPLETED_WITH_WARNINGS:
        ...
        return OperationReport(
            ...
            summary=push_completed_with_skips_summary(written=written, skipped=skipped),
...
def build_recovery_preview(validated: ResolvedRuntime) -> RecoveryPreview:
    ...
    if inert_unfinished:
        warnings.append(
            "No file writes started for this transaction. Discard metadata to unlock writes."
        )
    return RecoveryPreview(
        ...
        journal_summary=journal_payload(journal),
        detail=(
            "No file writes started — discard recovery metadata to unlock."
            if inert_unfinished
            else None
        ),
    )
