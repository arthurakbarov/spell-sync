...
def format_pull_planned_actual_lines(
    ...
    lines = format_aligned_fields(
        [
            ...
            ("Actual additions", actual),
            ("Skipped sources", skipped),
        ]
    )
    if actual != planned and preview.sources_skipped:
        detail_by_name = {row.name: row.detail for row in preview.source_rows}
        for name in preview.sources_skipped:
            reason = detail_by_name.get(name) or "unreadable"
            notice = build_notice(
                skip_reason_to_notice_code(reason),
                target_id=_target_family(name),
            )
...
def _planned_target_updates(preview: PushPreview) -> tuple[TargetUpdateReport, ...]:
    ...
    rows.extend(
        TargetUpdateReport(name=name, additions=0, removals=0, status="Skipped")
        for name in preview.skipped
    )
    rows.extend(
        TargetUpdateReport(name=name, additions=0, removals=0, status="Corrupt")
        for name in preview.corrupt
    )
    return tuple(rows)
