...
def format_words_to_add_line(preview: PushPreview) -> str:
    return f"Words to add: {len(unique_addition_words(preview))}"


def format_words_to_remove_line(preview: PushPreview) -> str:
    return f"Words to remove: {len(unique_removal_words(preview))}"
