...
from ..io import wordlist_unreadable, write_text_exclusive
...
def _support_notice(code: str) -> SupportNotice:
...
def build_support_report(
    ...
    *,
    ...
    path = Path(run.wordlist_str)
    wordlist_broken = wordlist_unreadable(path)
    word_count = None if wordlist_broken or not path.is_file() else len(run.load_wordlist())
    ...
    if wordlist_broken:
        notices.append(_support_notice("unreadable_wordlist"))
    if pending:
        notices.append(_support_notice("pending_recovery"))
    if journal.status is JournalLoadStatus.VALID_COMPLETED:
        notices.append(_support_notice("cleanup_pending"))
    if not config_valid:
        notices.append(_support_notice("invalid_config"))
...
def format_support_report_text(report: SupportReport) -> str:
    ...
    if report.notices:
        ...
        lines.extend(f"  {notice.title}: {notice.explanation}" for notice in report.notices)
    lines.extend(
        [
            "",
            "Privacy",
