...
class JournalLoadStatus(StrEnum):
    ABSENT = "absent"
    ...
    *,
    ...
    if not existed_before:
        ...
        return RecoveryItemPlan(
            ...
            # Newly created files need no snapshot — delete is the recovery action.
            status="ready",
            detail=None,
            existed_before=False,
            write_started=write_started,
...
def _parse_journal_dict(
    ...
    *,
    ...
    target_dicts: list[ParsedJournalTarget] = []
    ...
    targets = [
        JournalTarget(
            name=t["name"],
            path=t["path"],
            hash_before=t["hash_before"],
            hash_after=t["hash_after"],
            backup_path=t["backup_path"],
            existed_before=t["existed_before"],
            write_started=t["write_started"],
            write_completed=t["write_completed"],
        )
    ...
    if (
        wl_state["existed_before"]
        and wl_state["write_started"]
        and wl_state["backup_path"]
        and require_snapshots
    ):
    ...
    return PushJournal(
        ...
        state=state,
        wordlist=wordlist,
        wordlist_hash_before=wl_state["hash_before"],
        wordlist_hash_after=wl_state["hash_after"],
        wordlist_backup_path=wl_state["backup_path"],
        wordlist_existed_before=wl_state["existed_before"],
        wordlist_write_started=wl_state["write_started"],
        wordlist_write_completed=wl_state["write_completed"],
        snapshot_dir=snapshot_dir,
        targets=targets,
    )
    ...
    *,
    ...
    try:
        ...
        if exc.code == "missing_file":
            return JournalLoadResult(JournalLoadStatus.ABSENT, None)
        return JournalLoadResult(JournalLoadStatus.UNSAFE_ARTIFACT, None, exc.code)
    except OSError:
        return JournalLoadResult(JournalLoadStatus.CORRUPT, None, "unreadable")
...
class PushJournalSession:
    ...
    def complete(self) -> None:
        ...
        try:
            ...
            log.warn(
                "journal cleanup warning — completed journal left on disk. "
                "Inspect `.spell-sync.journal.json` carefully."
            )

    def discard(self) -> None:
        with contextlib.suppress(SecureArtifactError, OSError):
            remove_trusted_file(self._path, root=trusted_project_root(self._wordlist))
    ...
    def restore_one(
        ...
        *,
        ...
        if destination.is_file():
            current = file_content_hash(destination)
            if current is None:
                # Exists but unreadable — do not treat as missing and overwrite.
                failed.append(label)
                return
        else:
            current = None
        if current == hash_before:
            skipped.append(label)
            return
        if current == hash_after or not destination.is_file():
            pass
...
def cleanup_after_successful_recovery(journal: PushJournal) -> DiscardArtifactsResult:
    ...
    Snapshot paths are validated first so unsafe targets (for example a symlink)
    refuse cleanup while the journal still exists. Journal is then removed before
    snapshots so a failed journal unlink cannot leave a journal that requires
    already-deleted snapshots (false corrupt).
    """
    wordlist = Path(journal.wordlist)
    try:
        _safe_txn_snapshot_dir(wordlist, journal.transaction_id, journal.snapshot_dir)
    except DiscardSafetyError as exc:
        return DiscardArtifactsResult(False, False, str(exc))

    journal_ok, journal_detail = safe_discard_journal_file(wordlist)
    if not journal_ok:
        return DiscardArtifactsResult(False, False, journal_detail)
    ...
    if not snapshots_ok:
        return DiscardArtifactsResult(True, False, snap_detail)
...
def _safe_txn_snapshot_dir(
    ...
    try:
        ...
        raise DiscardSafetyError("snapshot directory is unreadable") from exc
    if resolved != expected_resolved:
        raise DiscardSafetyError("snapshot directory outside expected transaction directory")
    if not snap.is_dir():
        # Idempotent cleanup: snapshots may already be gone after a prior
        # successful discard that left the completed journal behind.
        if snap.exists():
            raise DiscardSafetyError("snapshot path is not a directory")
        return None
    return snap
...
def safe_discard_txn_snapshots(
    ...
    try:
        ...
        return False, "snapshot cleanup failed"
...
def discard_completed_journal(wordlist: Path) -> DiscardArtifactsResult:
    ...
    Snapshot paths are validated first so unsafe targets refuse cleanup while the
    journal still exists. Journal is then removed before snapshots so a failed
    journal unlink cannot leave a journal that still requires already-deleted
    snapshots.
    """
