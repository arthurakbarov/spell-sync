...
class ParsedJournalTarget(TypedDict):
    """Validated fields for one journal target entry."""
...
class ParsedWordlistState(TypedDict):
    """Validated wordlist-side fields shared by journal schema versions."""
...
def _validate_target_path(path: str) -> None:
    ...
    if ".." in Path(path).parts:
        raise JournalParseError("unsafe target path")


def parse_journal_target(item: dict[str, Any]) -> ParsedJournalTarget:
    name = parse_non_empty_str(item.get("name"), field="target.name")
    path = parse_non_empty_str(item.get("path"), field="target.path")
...
def parse_wordlist_state(raw: dict[str, Any]) -> ParsedWordlistState:
    existed_before = parse_bool_field(
...
def validate_journal_provenance(
    *,
    ...
    if expected_wordlist is not None and Path(wordlist).resolve() != expected_wordlist.resolve():
