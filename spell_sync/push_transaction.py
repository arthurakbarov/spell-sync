...
class TargetWriteState(StrEnum):
...
def _recovery_snapshot(path: Path, snapshot_dir: Path, *, root: Path, label: str) -> _FileBackup:
    ...
    if not is_path_readable(target):
        log.warn(f"backup skipped ({label}): read failed (path permissions)")
    ...
    try:
        ...
        log.warn(f"backup skipped ({label}): recovery snapshot not created")
        return _FileBackup(path, None, True, label)
...
def _plan_backup_path(path: Path, temp_dir: Path, *, label: str = "plan") -> _FileBackup:
    ...
    try:
        ...
        log.warn(f"backup skipped ({label}): temp backup not created")
        return _FileBackup(path, None, True, label)
...
def dictionaries_ready_to_write(
    ...
    for dictionary, bak in zip(dictionaries, backups, strict=True):
...
def _rollback_one_backup(bak: _FileBackup) -> bool:
    ...
    try:
        ...
        if bak.existed_before:
            log.warn(f"rollback skipped ({bak.label}) — no snapshot was made, file left unchanged")
        ...
        log.warn(f"rollback failed ({bak.label}).")
        bak.write_state = TargetWriteState.ROLLBACK_FAILED
        return False
