...
from .guest_messages import (
    PUSH_JOURNAL_CREATE_FAILED,
    PUSH_JOURNAL_CREATE_FAILED_LEFTOVER,
    PUSH_JOURNAL_FINALIZE_FAILED,
    PUSH_JOURNAL_UPDATE_FAILED,
    PUSH_TRANSACTION_START_FAILED,
    PUSH_WORDLIST_WRITE_FAILED,
)
...
def prepare_push(
    ...
    *,
    ...
    return PreparedPush(
        ctx=ctx,
        runtime_identity=build_runtime_identity(ctx),
        plan=plan,
        ...
        wordlist_hash=file_content_hash(ctx.wordlist),
    )
...
def plan_fingerprint_conflict(prepared: PreparedPush) -> str | None:
    ...
    if prepared.wordlist_hash is not None:
        current = file_content_hash(prepared.ctx.wordlist)
        if current != prepared.wordlist_hash:
            return prepared.ctx.wordlist.name
...
def write_rendered(
    ...
    *,
    ...
    try:
        ...
        log.warn(f"no write access ({path.name})")
        return False
...
def _abort_journal_begin_failure(tx: PushTransaction, exc: OSError) -> PushAbort:
    ...
    message = PUSH_JOURNAL_CREATE_FAILED
    if not cleanup_ok:
        reason = "journal_begin_failed"
        message = PUSH_JOURNAL_CREATE_FAILED_LEFTOVER
        return PushAbort(
            ExitCode.PUSH_ABORT,
            reason,
            message,
...
def execute_prepared_push(
    ...
    *,
    ...
    try:
        ...
        log.abort(PUSH_TRANSACTION_START_FAILED)
        return PushAbort(
            ExitCode.PUSH_ABORT,
    ...
    try:
        ...
        # Recheck wordlist immediately before mutating — fingerprints were frozen earlier.
        late_conflict = plan_fingerprint_conflict(prepared)
        if late_conflict is not None:
            return handle_failed_push_rollback(
                ...
                message=(
                    f"push aborted — {late_conflict} changed after plan (fingerprint conflict)."
                ),
            )
        ...
        if prepared.wordlist_needs_write and prepared.wordlist_rendered is not None:
            try:
                ...
                return handle_failed_push_rollback(
                    ...
                    message=PUSH_JOURNAL_UPDATE_FAILED,
            ...
            if not write_rendered(
                ...
                return handle_failed_push_rollback(
                    ...
                    message=PUSH_WORDLIST_WRITE_FAILED,
                )
            try:
        ...
        try:
            ...
            return handle_failed_push_rollback(
                ...
                journal_session,
                reason="journal_update_failed",
                message=PUSH_JOURNAL_FINALIZE_FAILED,
                journal_update_failed=True,
            )
