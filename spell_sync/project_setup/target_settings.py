...
from ..guest_messages import TARGET_SETTINGS_WRITE_FAILED
...
class PreparedTargetSettingsUpdate:
    ...
    can_execute: bool
    excluded_dictionary_names: frozenset[str] = frozenset()
    previous_excluded_dictionary_names: frozenset[str] = frozenset()


class TargetSettingsOutcome(StrEnum):
    COMPLETED = "completed"
    ...
    *,
    ...
    event_sink(
        build_technical_event(
            ...
            stage=EventStage.COMPLETED if terminal else stage,
...
def resolve_enabled_targets(
    ...
    *,
    ...
    discovered_ids = {
        target.identifier for target in discovery_targets if target.identifier in _CONFIG_TARGET_IDS
    }
    # Previously enabled families that are temporarily absent from discovery
    # (I/O blip, empty path probe) must not be written false. Uninstall without
    # a visible row would otherwise silently disable them on Review→Save.
    enabled: set[str] = {
        target_id
        for target_id in previous_target_ids
        if target_id in _CONFIG_TARGET_IDS and target_id not in discovered_ids
    }
...
def _update_id(
    *,
    ...
    for target_id in sorted(selected_target_ids):
        digest.update(target_id.encode("utf-8"))
    for name in sorted(excluded_dictionary_names):
...
def _excluded_from_loaded_config(config: dict[str, dict[str, Any]] | None) -> frozenset[str]:
    if config is None:
        return frozenset()
    value = config.get("dictionaries", {}).get("excluded")
...
def load_target_settings_snapshot(*, wordlist: Path) -> TargetSettingsSnapshot:
    ...
    excluded = _excluded_from_loaded_config(config)
    ...
    return TargetSettingsSnapshot(
        ...
        excluded_dictionary_names=excluded,
    )
    ...
    *,
    ...
    if config_result.status is ConfigStatus.ABSENT:
        return PreparedTargetSettingsUpdate(
            ...
            warnings=(*warnings, "spell-sync.toml is missing."),
            can_execute=False,
        )
    ...
    config = config_result.config or {}
    previous = _enabled_from_loaded_config(config)
    previous_excluded = _excluded_from_loaded_config(config)
    discovery = discover_setup_targets(
        enabled_targets=previous,
    )
    ...
    # Keep excludes only for dictionaries in enabled families that still exist.
    enabled_family_names = {
        name
        for target in discovery.targets
        if target.identifier in enabled
        for name, _count in target.dictionary_word_counts
    }
    excluded = frozenset(
        name
        for name in excluded_dictionary_names
        if name in known_dictionary_names and name in enabled_family_names
    )
    ...
    draft = ProjectConfigDraft(
        ...
        excluded_dictionaries=tuple(sorted(excluded)),
    )
    ...
    if enabled == previous and excluded == previous_excluded:
    ...
    return PreparedTargetSettingsUpdate(
        ...
        excluded_dictionary_names=excluded,
        previous_excluded_dictionary_names=previous_excluded,
    )
    ...
    *,
    ...
    try:
        with acquire_operation_lock(prepared.wordlist_path, "targets"):
            emit(EventId.TARGETS_LOCK_ACQUIRED, stage=EventStage.EXECUTING)
            ...
            emit(EventId.TARGETS_CONFLICTS_CHECKED, stage=EventStage.EXECUTING)
            if not _fingerprint_matches(prepared.config_path, prepared.config_fingerprint_before):
                return _return_with_terminal(
                    event_sink,
                    update_id=update_id,
            ...
            emit(EventId.TARGETS_VERIFYING, stage=EventStage.FINALIZING)
        ...
        return _return_with_terminal(
            ...
            execution=TargetSettingsExecution(
                prepared=prepared,
                outcome=TargetSettingsOutcome.FAILED,
                message=TARGET_SETTINGS_WRITE_FAILED,
                warnings=prepared.warnings,
            ),
            event_id=EventId.TARGETS_FAILED,
