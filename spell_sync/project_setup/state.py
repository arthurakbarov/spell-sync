...
from ..guest_messages import EXISTING_WORD_LIST_PREFIX, SETUP_WORD_LIST_UNREADABLE
...
class ProjectSetupStatus(StrEnum):
    READY = "ready"
    ...
    *,
    ...
    if wordlist_exists and wordlist_unreadable(wordlist):
        return ProjectSetupState(
            ...
            can_start_wizard=False,
            detail=SETUP_WORD_LIST_UNREADABLE,
            diagnostics=(
                ProjectSetupDiagnostic(
...
def inspect_existing_wordlist(path: Path) -> tuple[int | None, ReadStatus | None, str | None]:
    ...
    if wordlist_unreadable(path):
        return None, ReadStatus.UNREADABLE, SETUP_WORD_LIST_UNREADABLE
...
def validate_setup_wordlist(raw_path: str) -> tuple[Path, str | None]:
    ...
    if status is ReadStatus.UNREADABLE:
        raise ValueError(error or SETUP_WORD_LIST_UNREADABLE)
    if count is not None:
        detail_parts.append(f"{EXISTING_WORD_LIST_PREFIX} {count} words")
    else:
