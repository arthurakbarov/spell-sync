...
from ..guest_messages import SETUP_FILE_EXISTS, SETUP_WRITE_FAILED
...
class ProjectSetupOutcome(StrEnum):
    COMPLETED = "completed"
...
def _emit_setup_event(
    ...
    *,
    ...
    if event_sink is None:
        return
    terminal_stage = EventStage.COMPLETED if terminal else stage
    terminal_outcome = (
...
def execute_project_setup(
    ...
    *,
    ...
    emit(EventId.SETUP_VALIDATING, stage=EventStage.PREPARING)
    ...
    def _preflight_stop() -> ProjectSetupExecution | None:
        ...
        for item in prepared.files:
            ...
            if (
                item.action is SetupFileAction.KEEP
                and item.fingerprint is not None
                and not _fingerprint_matches(item.path, item.fingerprint)
            ):
                return _return_with_terminal(
                    event_sink,
                    setup_id=setup_id,
    ...
    try:
        with acquire_operation_lock(prepared.wordlist_path, "setup"):
            emit(EventId.SETUP_LOCK_ACQUIRED, stage=EventStage.EXECUTING)
            locked_stop = _preflight_stop()
            if locked_stop is not None:
                return locked_stop
            emit(EventId.SETUP_CONFLICTS_CHECKED, stage=EventStage.EXECUTING)
            ...
            emit(EventId.SETUP_VERIFYING, stage=EventStage.FINALIZING)
        ...
        leftover = _rollback_created(created)
        return _return_with_terminal(
            event_sink,
            setup_id=setup_id,
            execution=ProjectSetupExecution(
                prepared=prepared,
                outcome=ProjectSetupOutcome.STOPPED_SAFELY,
                message=SETUP_FILE_EXISTS,
                created_files=tuple(path.name for path in leftover),
            ),
            event_id=EventId.SETUP_STOPPED_SAFELY,
        ...
        leftover = _rollback_created(created)
        outcome = ProjectSetupOutcome.SETUP_INCOMPLETE if leftover else ProjectSetupOutcome.FAILED
        return _return_with_terminal(
            event_sink,
            setup_id=setup_id,
            execution=ProjectSetupExecution(
                prepared=prepared,
                outcome=outcome,
                message=SETUP_WRITE_FAILED,
                created_files=tuple(path.name for path in leftover),
            ),
            event_id=EventId.SETUP_INCOMPLETE if leftover else EventId.SETUP_FAILED,
    ...
    message = (
        "Project created. The existing personal word list was kept unchanged."
...
def _rollback_created(paths: list[Path]) -> list[Path]:
    """Unlink created files; return paths that remain (true incomplete leftovers)."""
    leftover: list[Path] = []
