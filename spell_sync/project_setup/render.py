...
def render_project_config(
    ...
    *,
    ...
    lines = ["[dictionaries]"]
    lines.extend(
        f"{name} = {'true' if name in enabled else 'false'}" for name in _ALL_DICTIONARY_FLAGS
    )
    excluded = sorted({name for name in draft.excluded_dictionaries if name})
    if excluded:
        rendered = ", ".join(_toml_string(name) for name in excluded)
        lines.append(f"excluded = [{rendered}]")
    lines.append("")
    lines.append("[push]")
    ...
    if existing_config is not None:
        ...
        if isinstance(max_removals, int) and not isinstance(max_removals, bool):
