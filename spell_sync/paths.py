...
def find_project_wordlist() -> Path | None:
    """Wordlist path when cwd (or an ancestor) has a real project marker file.

    Returns ``directory / wordlist.txt`` when that directory contains
    ``wordlist.txt`` and/or ``spell-sync.toml``. Does not fall back to bare cwd.
    """
    cwd = Path.cwd()
    for directory in (cwd, *cwd.parents):
        wordlist = directory / WORDLIST_FILENAME
        config = directory / "spell-sync.toml"
        if wordlist.is_file() or config.is_file():
            return wordlist
    return None
...
def resolve_wordlist_path(explicit: str | None = None) -> Path:
    """Resolve wordlist: explicit → cwd project → machine pointer → cwd default.

    Machine pointer is last-used state (see ``project_memory``), not project config.
    """
    if explicit:
        return Path(explicit).expanduser()
    found = find_project_wordlist()
    if found is not None:
        return found
    from .project_memory import remembered_wordlist

    remembered = remembered_wordlist()
...
def neovim_dict_paths() -> list[PathPair]:
    # Guest label is "nvim" (full personal wordlist). The on-disk basename
    # en.utf-8.add is Neovim's spell-file convention, not a language filter.
    spell_dir = neovim_data_dir() / "site" / "spell"
    return [("nvim", spell_dir / "en.utf-8.add")]
