...
def git_hooks_checks(status: GitHooksStatus) -> list[DoctorCheck]:
    ...
    if missing:
        checks.append(
            DoctorCheck(
                ...
                "Install git hooks in your word list folder.",
            ),
        )
    if status.pre_push_stale:
        checks.append(
            DoctorCheck(
                "warn",
                "pre-push hook is outdated — reinstall git hooks in your word list folder.",
            ),
        )
    if status.pre_commit_stale:
        checks.append(
            DoctorCheck(
                "warn",
                "pre-commit hook is outdated — reinstall git hooks in your word list folder.",
            ),
        )
    return checks
