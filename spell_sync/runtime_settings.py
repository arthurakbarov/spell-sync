...
class RuntimeSettings:
    ...
    def from_config_dict(cls, config: Mapping[str, Mapping[str, Any]]) -> RuntimeSettings:
        ...
        return cls(
            ...
            push=PushPolicy(
                guard_wordlist_max=_non_negative_int(
                    push, "guard_wordlist_max", PUSH_GUARD_WORDLIST_MAX
                ),
                guard_local_min=_non_negative_int(push, "guard_local_min", PUSH_GUARD_LOCAL_MIN),
                strict=_bool(push, "strict", False),
                max_removals_without_confirm=_non_negative_int(
                    push,
...
def _non_negative_int(section: Mapping[str, Any], key: str, default: int) -> int:
    """Config int that reverts to the (protective) default when negative.

    A negative safety threshold is nonsensical and would silently disable the
    guard it belongs to, so fall back to the default rather than trust it.
    """
