...
from .guest_messages import (
    PUSH_DICTIONARY_BACKUPS_FAILED,
    PUSH_WORDLIST_BACKUP_FAILED,
    WORD_LIST_NOT_FOUND,
    WORD_LIST_UNREADABLE,
)
from .io import is_path_writable, read_text_words, wordlist_unreadable
from .log import log
...
def require_wordlist_readable(
    ctx: RuntimeContext, *, allow_missing: bool = False
) -> ExitCode | None:
    """Fail closed unless the personal word list is a readable file.

    ``allow_missing`` is for Collect: a missing list may be created from app
    dictionaries. Update and Status must not treat missing as empty.
    """
    path = Path(ctx.wordlist_str)
    if not path.is_file():
        if allow_missing:
            return None
        log.abort(WORD_LIST_NOT_FOUND)
        return ExitCode.WORDLIST_UNREADABLE
    if wordlist_unreadable(path):
        log.abort(f"{WORD_LIST_UNREADABLE} {TCC_ACCESS_HINT}")
        return ExitCode.WORDLIST_UNREADABLE
    return None


def verify_pull_source_fingerprints(
    source_rows: Iterable[PullSourcePreview],
) -> str | None:
    """Return source name when a used pull source diverged since preview."""
    ...
    for row in source_rows:
        ...
        name = row.name or path_value
        if expected is None:
            # Source was missing at preview — refuse if it appeared since then.
...
def prepare_writable_dictionaries(
    ...
    tx: PushTransaction,
    dictionaries: list[Dictionary],
) -> tuple[list[Dictionary], set[str]] | ExitCode:
    if tx.wordlist_backup.existed_before and tx.wordlist_backup.backup is None:
        log.abort(PUSH_WORDLIST_BACKUP_FAILED)
    ...
    if dictionaries and not writable:
        log.abort(PUSH_DICTIONARY_BACKUPS_FAILED)
        return ExitCode.PUSH_ABORT

    skipped = {d.name for d in dictionaries if d not in writable}
    ...
    for dictionary in writable:
        if is_path_writable(dictionary.path):
            ready.append(dictionary)
            continue
        skipped.add(dictionary.name)
        warn_dictionary_skipped(dictionary.name, "not writable — push skipped")

    if dictionaries and not ready:
        blocked = strict_push_blocked(ctx, sorted(skipped), "not writable")
        if blocked is not None:
            return blocked
        return ready, skipped

    blocked = strict_push_blocked(ctx, sorted(skipped), "backup failed")
    if blocked is not None:
        return blocked
    return ready, skipped
