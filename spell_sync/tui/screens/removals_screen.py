...
from ...application.product_concepts import numbered_word_lines, words_count_label
...
class RemovalsScreen(Screen[None]):
    ...
    def on_mount(self) -> None:
        ...
        summary = self._summary or f"{self._title}: {words_count_label(count)}"
        self.query_one("#removals-summary", Static).update(summary)
        if self._body is not None:
            body = self._body
        elif count:
            body = numbered_word_lines(self._removal_words)
        else:
