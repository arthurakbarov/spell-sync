...
from ...application.product_concepts import HEALTH_LABEL
...
class DoctorScreen(LoadTokenMixin, Screen[None]):
    ...
    def on_mount(self) -> None:
        ...
        try:
            ...
            self.query_one("#doctor-summary", Static).update(f"× {HEALTH_LABEL} load failed.")
    ...
    def _render_snapshot(self, snapshot: DoctorSnapshot) -> None:
        ...
        if table.row_count == 0 and not snapshot.has_errors:
            table.add_row("—", "✓ Passed", "No issues", "Health checks look clear.")
        if snapshot.has_errors:
            overall = "Overall: × blocking issues found"
        else:
            overall = "Overall: ✓ no blocking errors"
        self.query_one("#doctor-summary", Static).update(f"{HEALTH_LABEL}\n{overall}")
        sync_data_table_rows(table)
    ...
    def run_doctor(self) -> None:
        ...
        self.query_one("#doctor-summary", Static).update(
            loading_message(f"Running {HEALTH_LABEL}...", "doctor")
        )
    ...
    def on_load_doctor_worker_state_changed(self, event) -> None:
        ...
        if event.state is WorkerState.ERROR:
            if self.is_mounted:
                self.query_one("#doctor-summary", Static).update(
                    f"× {HEALTH_LABEL} unavailable — try Run again."
                )
                table = self.query_one("#doctor-table", DataTable)
                table.clear()
                sync_data_table_rows(table)
    ...
    def _export_support_report(self) -> None:
        ...
        set_optional_static(
            self.query_one("#doctor-export-status", Static),
    ...
    def _finish_export_worker(self, worker: Worker | None) -> None:
        ...
        status = self.query_one("#doctor-export-status", Static) if self.is_mounted else None
        if worker.state is WorkerState.ERROR:
            if status is not None:
                set_optional_static(status, "× Support report could not be exported.")
        ...
        set_optional_static(
            status,
            f"× {export_result.message or 'Support report could not be exported.'}",
        )
