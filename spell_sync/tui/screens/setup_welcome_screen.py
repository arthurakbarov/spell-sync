...
from ...guest_messages import SETUP_WORD_LIST_UNREADABLE
...
class SetupOpenProjectScreen(_PathCompleteMixin, Screen[None]):
    ...
    def on_radio_set_changed(self, event: RadioSet.Changed) -> None:
        ...
        if (
            pressed is not None
            and pressed.id
            and pressed.id.startswith("recent-")
            and pressed.id != "recent-other"
        ):
            try:
    ...
    def on_mount(self) -> None:
        ...
        for item in prepared.files:
            if item.action.value == "create":
                ...
                if (
                    item.path == prepared.wordlist_path
                    and SETUP_WORD_LIST_UNREADABLE in prepared.conflicts
                ):
                    lines.append(f"  {item.relative_name} (cannot be read — blocks create)")
                else:
