...
class DashboardScreen(LoadTokenMixin, Screen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="screen-body", classes="screen-body"):
            ...
            with Vertical(id="screen-actions", classes="dashboard-menu"):
                ...
                yield menu_item(
                    Button(ADD_WORDS_LABEL, id="btn-add-words"),
                    ADD_WORDS_HELP,
                    hint_id="add-words-hint",
                )
                yield menu_item(
                ...
                yield menu_item(
                    Button(HEALTH_LABEL, id="btn-health"),
                    HEALTH_BUTTON_HELP,
                    hint_id="health-hint",
                )
                yield menu_item(
    ...
    def _refresh_next_step(
        self,
        *,
        apps_configured: bool | None = None,
        ...
        if apps_configured is False:
            ...
            return
        if empty_wordlist:
            self._set_optional_static("#dashboard-next-step", DASHBOARD_EMPTY_WORDLIST_CTA)
            return
    ...
    def _format_summary(self, state) -> str:
        ...
        if snapshot.wordlist_error is not None:
            word_count_line = WORD_LIST_COUNT_UNREADABLE
        else:
            word_count_line = f"{snapshot.wordlist_count:,} words"
        # Same outline for every group: heading, then indented facts.
        indent = "    "
        lines = [
            DASHBOARD_WORDLIST_LABEL,
            f"{indent}{_display_path(state.wordlist_path)}",
            f"{indent}{word_count_line}",
            "",
            APPLICATIONS_LABEL,
        ]
        tallies: list[str] = []
    ...
    def _render_dashboard(self, state) -> None:
        ...
        if state.issues:
            ...
            for issue in state.issues:
                ...
                issue_lines.append(f"{indent}{prefix} {format_notice_summary(notice)}")
                issue_lines.append(f"{indent}{indent}{format_notice_details(notice)}")
        ...
        recovery_needed = state.pending_recovery or any(
            issue.code in {"pending_recovery", "corrupt_journal", "cleanup_pending"}
            for issue in state.issues
        )
        cleanup_only = any(issue.code == "cleanup_pending" for issue in state.issues) and not (
            state.pending_recovery
            or any(issue.code in {"pending_recovery", "corrupt_journal"} for issue in state.issues)
        )
        apps_configured = _apps_configured(state)

        review_btn = self.query_one("#btn-review-update", Button)
        recovery_btn = self.query_one("#btn-recovery", Button)
        add_words_btn = self.query_one("#btn-add-words", Button)
        pull_btn = self.query_one("#btn-pull", Button)
        extra_words_btn = self.query_one("#btn-extra-words", Button)
        push_btn = self.query_one("#btn-push", Button)
        targets_btn = self.query_one("#btn-targets", Button)

        pull_btn.disabled = self._blocked
        extra_words_btn.disabled = self._blocked
        push_btn.disabled = self._blocked
        add_words_btn.disabled = self._blocked
        empty_wordlist = bool(state.snapshot.empty_wordlist)

        if recovery_needed:
            review_btn.disabled = True
            review_btn.variant = "default"
            add_words_btn.variant = "default"
            self._set_recovery_item_visible(True)
            recovery_btn.disabled = False
            recovery_btn.variant = "primary"
            recovery_btn.label = (
                RECOVERY_CLEANUP_LABEL if cleanup_only else "Finish interrupted update"
            )
            ...
            review_btn.disabled = self._blocked
            review_btn.variant = "default"
            add_words_btn.variant = "primary"
            self._set_recovery_item_visible(False)
            recovery_btn.disabled = True
            recovery_btn.variant = "default"
            targets_btn.variant = "default"
            self._refresh_next_step(apps_configured=True, empty_wordlist=True)
        else:
            review_btn.disabled = self._blocked
    ...
    def on_button_pressed(self, event: Button.Pressed) -> None:
        ...
        elif button_id == "btn-add-words":
            self.action_open_add_words()
        elif button_id == "btn-pull":
            self.action_open_pull()
        elif button_id == "btn-extra-words":
    ...
    def action_open_add_words(self) -> None:
        if self._blocked:
            self.notify("Writes are blocked. Resolve recovery or blocking issues first.")
            return
        from .first_win_screen import AddWordsScreen
