...
def _format_pull_preview(preview: PullPreview) -> str:
    ...
    if preview.additions == 0:
        lines.extend(["", PULL_PREVIEW_EMPTY_NEXT])
    else:
        lines.append("")
        lines.extend(
            format_aligned_fields(
                [
                    ("Dictionaries ready", len(preview.sources_used)),
                    ("Dictionaries skipped", len(preview.sources_skipped)),
                ]
            )
        )
    if preview.warnings:
...
class ReviewPullScreen(LoadTokenMixin, Screen[None]):
    ...
    def _render_preview(self, preview: PullPreview) -> None:
        ...
        # Empty collect: hide View additions and dead Collect; the only forward step is
        # Update (not an optional "skip" — say Continue).
...
class ReviewPullCompleteScreen(Screen[None]):
    ...
    def compose(self) -> ComposeResult:
        ...
        with VerticalScroll(id="screen-body", classes="screen-body"):
            ...
            yield action_bar(
                Button(
                    f"Build {UPDATE_APPS_LABEL} preview",
                    id="btn-build-push",
                    variant="primary",
                )
            )
...
class ReviewSessionReportScreen(LoadTokenMixin, Screen[None]):
    ...
    def on_mount(self) -> None:
        ...
        lines.extend(
            [
                "",
                (REVIEW_SESSION_DONE_MATCHED if report.is_matched else REVIEW_SESSION_DONE_PARTIAL),
            ]
        )
