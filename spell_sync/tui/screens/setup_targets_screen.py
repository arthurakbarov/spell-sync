...
class SetupTargetsScreen(Screen[None]):
    ...
    def _compact_header(self) -> str:
        ...
        for index, line in enumerate(lines):
            ...
            elif stripped.startswith("Existing wordlist:"):
                words = stripped.removeprefix("Existing wordlist:").strip()
        ...
        summary = " · ".join(parts) if parts else "Choose apps to sync"
    ...
    def _focusable_rows(self) -> list[SetupTargetRowWidget | DictionaryIncludeRowWidget]:
        container = self.query_one("#targets-list", Vertical)
        return [
            child
