...
from .application.product_concepts import WORD_LIST_COUNT_UNREADABLE
    ...
    with quiet_json_output(opts):
        ...
        if request.list_targets:
            with operation_session(
                OperationSpec(
                    key="doctor",
                    title="doctor: discovered app dictionary paths",
                    descriptions=(
                ...
                if not targets:
                    if session is not None:
                        session.succeed("no application dictionaries discovered for this platform")
                    else:
        ...
        with operation_session(
            OperationSpec(
                key="doctor",
                title="doctor: paths, permissions, and drift",
                descriptions=(
            ...
            count = (
                ...
                if report.wordlist_count is None
                else f"{report.wordlist_count} words"
            )
            log.info(f"word list: {report.wordlist_path} ({count})")
            log.info(
            ...
            if report.wordlist_count is not None:
                log.info(
                    f"drift: up to +{report.max_drift_add} / -{report.max_drift_remove} "
                    "words on Update my apps"
                )
            for check in report.checks:
                if check.level == "error":
