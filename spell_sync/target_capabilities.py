...
class TargetFilterKind(StrEnum):
    FULL = "full"
    LATIN = "latin"
...
class TargetProfileModel(StrEnum):
    SINGLE = "single"
    MULTI_PROFILE = "multi-profile"
    SYSTEM_MANAGED = "system-managed"


class ApplicationClosePolicy(StrEnum):
...
TARGET_CAPABILITIES: tuple[TargetCapability, ...] = tuple(
    sorted(
        (
            ...
            _capability("sublime", platforms=_PLATFORM_ALL, custom_dictionary_kind="json"),
...
def registry_target_platform_pairs() -> tuple[tuple[str, str], ...]:
    return tuple(
        (capability.identifier, platform)
        for capability in TARGET_CAPABILITIES
