...
When setup finds a readable custom dictionary on your computer, that app is
**on by default**. You can turn apps off under **Applications** during or after setup.

Examples of what “found” usually means:

| App area | App id | Notes |
|----------|--------|-------|
| Chrome / Edge / Brave / Vivaldi | `chrome`, `edge`, ... | Chromium custom dictionary files |
| Firefox | `firefox` | `persdict.dat` profiles |
| Editors (VS Code / Cursor family) | `editors` | Maintains `spell-sync-words.txt` (wire your spell-check extension — see [Editors](#editors)) |
...
it appears under **Applications**. Run **Check my apps** to see what was found on your computer.
...
Open **Applications** in Spell Sync for your installation. The screen shows:
...
Chrome | macos | Yes | Yes | full | multi-profile | block-if-running | pass | pass (mutation-ui) | 2026-08-08
...
Editor dictionaries | macos | Yes | Yes | full | multi-profile | not-required | pass | pass (mutation-ui) | 2026-08-15
...
macOS Spelling | macos | Yes | Yes | full | system-managed | not-required | pass | pass (read-only) | 2026-08-05
...
Sublime Text | macos | Yes | Yes | full | multi-profile | not-required | pass | pass (mutation-ui) | 2026-08-15
...
Spell Sync maintains a plain-text file named `spell-sync-words.txt` in the editor’s
user data directory (for example under Application Support on macOS). On
**Update my apps**, that file is rewritten from your word list.

Spell Sync does **not** consume this file automatically inside the editor. VS Code and
Cursor do not treat it as a dictionary until a spell-check extension is configured to
use it (commonly [Code Spell Checker / cSpell](https://marketplace.visualstudio.com/items?itemName=streetsidesoftware.code-spell-checker)
via `cSpell.dictionaryDefinitions` pointing at `spell-sync-words.txt`, and
`cSpell.dictionaries` including that definition name).

### Recommendations

1. Keep personal vocabulary in your Spell Sync word list (`wordlist.txt`).
2. After first setup, point your spell-check extension at `spell-sync-words.txt` once.
3. Run **Update my apps**, then reload the editor window if the extension caches dictionaries.
4. Prefer adding and removing words through Spell Sync rather than only inside the extension UI, so the word list stays the source of truth.

### What Spell Sync never does for editors

- It does not install or configure spell-check extensions.
- It does not rewrite unrelated `settings.json` keys (themes, formatters, keybindings).
- It does not change built-in dictionaries shipped with the editor.
...
Manual rows are recorded in `docs/technical/target-validation.json` (including
`manual_scope`: `read-only`, `mutation`, or `mutation-ui` when `manual_validation`
is `pass`). Synthetic CI tests provide `automated_validation: pass` only.
...
- macOS and Windows system spelling apps are platform-specific and not available on all OSes.
- Editors: Spell Sync writes `spell-sync-words.txt` but does not auto-wire spell-check extensions —
  see [Editors](#editors).
