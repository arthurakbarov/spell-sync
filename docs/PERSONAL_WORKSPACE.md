...
| **Synced folder** (Dropbox, iCloud Drive, Yandex Disk, OneDrive, ...) | The sync app copies the folder between machines — same outcome as a remote repo, without Git. | Pause sync if two computers might edit during Update. Conflict copies from the sync app are outside Spell Sync. |
