...
Symptom index and next steps: [Troubleshooting](TROUBLESHOOTING.md).
Recovery detail: [Recovery](RECOVERY.md).
For target coverage notes, see [Supported apps](SUPPORTED_APPS.md).
...
Chrome may have been running (Update my apps skips running Chromium targets), the word may be outside
...
wordlist unless you also ran Collect my words.

### What to do next
Close Chrome completely, rebuild the Update preview, confirm, and run Update my apps again. Verify the target
...
### What happened
Update my apps or Status reports the Sublime target as up to date, but Sublime’s spell
...
### What was not changed
Your wordlist and project files.
...
uv tool install --force --python 3.14 git+https://github.com/arthurakbarov/spell-sync
...
### Why Spell Sync stopped or skipped
Nothing is wrong — preview is intentional. Push never runs automatically from
`status` or `plan`.
...
**Review and update** to review the Push preview before confirming.
