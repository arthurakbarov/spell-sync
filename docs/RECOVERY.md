...
1. Open the dashboard and look for **Finish interrupted update**, or run `spell-sync recover`.
2. If recovery is not offered and you only want to reverse a **successful** Update my apps,
   there is no `rollback` command — see
   [I updated my apps and want to undo it](TROUBLESHOOTING.md#i-updated-my-apps-and-want-to-undo-it).
...
Push may also leave rotating `*.bak` / `*.1.bak` / `*.2.bak` files beside dictionaries (`[io] backup_keep`). There is no
`rollback` CLI — restore from `.bak` manually if you need a pre-push copy outside the journal.

## Journal files

After Update starts, spell-sync may leave:
...
Windows). Only one command that changes files may run per word list at a time.
